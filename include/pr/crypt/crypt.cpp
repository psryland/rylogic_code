...
// CRC the contents of a file
CRC crypt::CrcFile(const char* filename, CRC initial_crc)
{
	...
	if (file == INVALID_HANDLE_VALUE) return initial_crc;

	uint8 buffer[4096];
	DWORD bytes_read;
	while (pr::FileRead(file, buffer, sizeof(buffer), &bytes_read))
		initial_crc = Crc(buffer, bytes_read, initial_crc);
