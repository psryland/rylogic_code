...
namespace pr::rdr
{
	...
	public:
			...
			,m_light(light)
		{
			...
			m_btn_preview.Click += [&,preview](gui::Button&, gui::EmptyArgs const&)
			{
				ReadValues();
				preview(m_light);
			};
		...
		{
			// Light type
			if (m_rdo_ambient    .Checked()) m_light.m_type = rdr::ELight::Ambient;
			if (m_rdo_directional.Checked()) m_light.m_type = rdr::ELight::Directional;
			if (m_rdo_point      .Checked()) m_light.m_type = rdr::ELight::Point;
