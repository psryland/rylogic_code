...
namespace pr::rdr
{
	class ShaderManager
	{
		...
		template <typename ShaderType> void CreateStockShader();
	...
	public:
		...
		RefPtr<ShaderType> FindShader(RdrId id)
		{
			...
			return RefPtr<ShaderType>(static_cast<ShaderType*>(shdr), true);
		}
		...
		template <typename ShaderType, typename = std::enable_if_t<std::is_base_of_v<Shader, ShaderType>>>
		RefPtr<ShaderType> GetShader(RdrId id, RdrId base_id, char const* name = nullptr)
		{
		...
		template <typename ShaderType, typename = std::enable_if_t<std::is_base_of_v<Shader, ShaderType>>>
		RefPtr<ShaderType> GetShader(char const* id, RdrId base_id, char const* name = nullptr)
		{
