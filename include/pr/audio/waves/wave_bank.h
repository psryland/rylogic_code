...
namespace pr
{
	namespace audio
	{
		...
		public:
			...
			// Open a wave bank
			void Open(wchar_t const* szFileName);
		...
		public:
			...
			void Write(char const* bank_name, wchar_t const* xwb_filepath, EOptions opts = EOptions::None) const;

			// Write a C header for the wave bank
			void WriteHeader(char const* bank_name, std::ostream& hdr) const;
			void WriteHeader(char const* bank_name, wchar_t const* header_filepath, EOptions opts = EOptions::None) const;
		};
	}
}
