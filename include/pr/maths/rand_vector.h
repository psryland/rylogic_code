...
namespace pr
{
	...
	inline v3 Random2N(Rand& rnd, float z_)
	{
		return v3(Random2N(rnd), z_);
	}
	inline v4 Random2N(Rand& rnd, float z_, float w_)
	{
		return v4(Random2N(rnd), z_, w_);
	}

	// Create a random vector with length on interval [min_length, max_length]
	inline v2 Random2(Rand& rnd, float min_length, float max_length)
	{
		return rnd.fltr(min_length, max_length) * Random2N(rnd);
	}
	inline v3 Random2(Rand& rnd, float min_length, float max_length, float z_)
	{
		return v3(Random2(rnd, min_length, max_length), z_);
	}
	inline v4 Random2(Rand& rnd, float min_length, float max_length, float z_, float w_)
	{
		return v4(Random2(rnd, min_length, max_length), z_, w_);
	}

	// Create a random vector with components on interval [vmin, vmax]
	inline v2 Random2(Rand& rnd, v2 const& vmin, v2 const& vmax)
	{
		return v2(rnd.fltr(vmin.x, vmax.x), rnd.fltr(vmin.y, vmax.y));
	}
	inline v3 Random2(Rand& rnd, v3 const& vmin, v3 const& vmax, float z_)
	{
		return v3(Random2(rnd, vmin.xy, vmax.xy), z_);
	}
	inline v4 Random2(Rand& rnd, v4 const& vmin, v4 const& vmax, float z_, float w_)
	{
		return v4(Random2(rnd, vmin.xy, vmax.xy), z_, w_);
	}

	// Create a random vector centred on 'centre' with radius 'radius'
	inline v2 Random2(Rand& rnd, v2 const& centre, float radius)
	{
		return Random2(rnd, 0.0f, radius) + centre;
	}
	inline v3 Random2(Rand& rnd, v3 const& centre, float radius, float z_)
	{
		return v3(Random2(rnd, centre.xy, radius), z_);
	}
	inline v4 Random2(Rand& rnd, v4 const& centre, float radius, float z_, float w_)
	{
		return v4(Random2(rnd, centre.xy, radius), z_, w_);
	}

	// Create a random vector with unit length
	inline v3 Random3N(Rand& rnd)
	{
		v3 v; float len;
		do
		{
	...
	inline v4 Random3N(Rand& rnd, float w_)
	{
		return v4(Random3N(rnd), w_);
	}
	
	// Create a random vector with length on interval [min_length, max_length]
	inline v3 Random3(Rand& rnd, float min_length, float max_length)
	{
		return rnd.fltr(min_length, max_length) * Random3N(rnd);
	}
	inline v4 Random3(Rand& rnd, float min_length, float max_length, float w_)
	{
		return v4(Random3(rnd, min_length, max_length), w_);
	}

	// Create a random vector with components on interval [vmin, vmax]
	inline v3 Random3(Rand& rnd, v3 const& vmin, v3 const& vmax)
	{
		return v3(rnd.fltr(vmin.x,vmax.x), rnd.fltr(vmin.y,vmax.y), rnd.fltr(vmin.z,vmax.z));
	}
	inline v4 Random3(Rand& rnd, v4 const& vmin, v4 const& vmax, float w_)
	{
		return v4(Random3(rnd, vmin.xyz, vmax.xyz), w_);
	}

	// Create a random vector centred on 'centre' with radius 'radius'
	inline v3 Random3(Rand& rnd, v3 const& centre, float radius)
	{
		return Random3(rnd, 0.0f, radius) + centre;
	}
	inline v4 Random3(Rand& rnd, v4 const& centre, float radius, float w_)
	{
		return v4(Random3(rnd, centre.xyz, radius), w_);
	}

	// Create a random vector with unit length
	inline v4 Random4N(Rand& rnd)
	{
		v4 v; float len;
		do
		{
	...
	inline m4x4 Random4x4(Rand& rnd, v4 const& axis, float min_angle, float max_angle, v4 const& position)
	{
		return m4x4::Rotation(axis, rnd.fltr(min_angle, max_angle), position);
	}
	inline m4x4 Random4x4(Rand& rnd, float min_angle, float max_angle, v4 const& position)
	{
		return Random4x4(rnd, Random3N(rnd, 0.0f), min_angle, max_angle, position);
	}
	inline m4x4 Random4x4(Rand& rnd, v4 const& axis, float min_angle, float max_angle, v4 const& centre, float radius)
	{
		return Random4x4(rnd, axis, min_angle, max_angle, centre + Random3(rnd, 0.0f, radius, 0.0f));
	}
	inline m4x4 Random4x4(Rand& rnd, float min_angle, float max_angle, v4 const& centre, float radius)
	{
		return Random4x4(rnd, Random3N(rnd, 0.0f), min_angle, max_angle, centre, radius);
	}
	inline m4x4 Random4x4(Rand& rnd, v4 const& centre, float radius)
	{
		return Random4x4(rnd, Random3N(rnd, 0.0f), 0.0f, maths::tau, centre, radius);
	}

	// Create a random quaternion rotation
	inline quat RandomQ(Rand& rnd, v4 const& axis, float min_angle, float max_angle)
	{
		return quat(axis, rnd.fltr(min_angle, max_angle));
	}
	inline quat RandomQ(Rand& rnd, float min_angle, float max_angle)
	{
		return quat(Random3N(rnd, 0.0f), rnd.fltr(min_angle, max_angle));
	}
	inline quat RandomQ(Rand& rnd)
	{
