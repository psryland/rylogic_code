...
namespace pr::maths
{
	PRUnitTest(Vector8Tests, float, double)
	{
		...
		{// LinAt, AngAt
			...
			PR_CHECK(FEql(lin0, vec4_t{0,0,0,0}), true);
			PR_CHECK(FEql(ang0, vec4_t{0,0,2,0}), true);

			auto lin1 = v.LinAt(vec4_t{0,0,0,0});
			auto ang1 = v.AngAt(vec4_t{0,0,0,0});
			PR_CHECK(FEql(lin1, vec4_t{0,1,0,0}), true);
			PR_CHECK(FEql(ang1, vec4_t{0,0,1,0}), true);

			auto lin2 = v.LinAt(vec4_t{+1,0,0,0});
			auto ang2 = v.AngAt(vec4_t{+1,0,0,0});
			PR_CHECK(FEql(lin2, vec4_t{0,2,0,0}), true);
			PR_CHECK(FEql(ang2, vec4_t{0,0,0,0}), true);

			auto lin3 = v.LinAt(vec4_t{+2,0,0,0});
			auto ang3 = v.AngAt(vec4_t{+2,0,0,0});
			PR_CHECK(FEql(lin3, vec4_t{0,3,0,0}), true);
			PR_CHECK(FEql(ang3, vec4_t{0,0,-1,0}), true);

			auto lin4 = v.LinAt(vec4_t{+3,0,0,0});
			auto ang4 = v.AngAt(vec4_t{+3,0,0,0});
			PR_CHECK(FEql(lin4, vec4_t{0,4,0,0}), true);
			PR_CHECK(FEql(ang4, vec4_t{0,0,-2,0}), true);
		}
		{// Projection
			...
			auto vt = v - vn;
			auto r = vn + vt;
			PR_CHECK(FEql(vn, vec8_t{0,0,3,0,0,-1}), true);
			PR_CHECK(FEql(vt, vec8_t{1,-2,0,-3,2,0}), true);
