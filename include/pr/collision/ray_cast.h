...
namespace pr
{
	namespace collision
	{
		...
		template <typename TShape, typename = enable_if_shape<TShape>> inline RayCastResult RayCastWS(Ray const& ray, TShape const& shape, m4x4 const& s2w)
		{
		...
		template <typename = void> inline RayCastResult RayCast(Ray const& ray, Shape const& shape)
		{
			switch (shape.m_type)
			{
			default: assert("Unknown primitive type" && false); return;
			case EShape::Sphere: return RayCast(ray, shape_cast<ShapeSphere>(shape));
			case EShape::Box:    return RayCast(ray, shape_cast<ShapeBox>   (shape));
			}
		}

		// Ray vs. Sphere
		template <typename = void> RayCastResult RayCast(Ray const& ray, ShapeSphere const& shape)
		{
			RayCastResult result;

			// Find the closest point to the line
			auto direction_lenSq = Length3Sq(ray.m_direction);
			auto closest_point   = ray.m_point - ray.m_direction * (Dot3(ray.m_direction, ray.m_point) / direction_lenSq);
			auto closest_distSq  = Length3Sq(closest_point);
		...
		// Ray vs. Box
		template <typename = void> RayCastResult RayCast(Ray const& ray, ShapeBox const& shape)
		{
			RayCastResult result;
		...
		// Ray vs. Triangle
		template <typename = void> RayCastResult RayCast(Ray const& ray, ShapeTriangle const& shape)
		{
			RayCastResult result;
			...
			auto t = Sqrt(Length3Sq(intercept - r.m_point) / Length3Sq(r.m_direction));
		...
		// Ray vs. Array
		template <typename = void> RayCastResult RayCast(Ray const& ray, ShapeArray const& shape)
		{
			RayCastResult result;
			result.m_t0 = 1.0f;
		...
		// Ray vs. Polytope
		template <typename = void> RayCastResult RayCast(Ray const& ray, ShapePolytope const& shape)
		{
