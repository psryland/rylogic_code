...
namespace pr::app
{
	...
	template<typename DerivedGUI, typename Main, typename MessageLoop = pr::gui::MessageLoop>
	struct MainGUI :pr::gui::Form ,IAppMainGui
	{
		...
		struct Params :pr::gui::MakeFormParams<FormParams>
		{
			Params()
			{
				wndclass(RegisterWndClass<DerivedGUI>())
					.main_wnd(true);
			}
		};
		...
		MainGUI(FormParams const& p)
			:Form(p)
			,m_log(Main::AppName(), log::ToFile(FmtS(L"%S.log", Main::AppName())), 0)
			,m_msg_loop()
