...
namespace pr
{
	...
	public:
		...
		bool Read(const char* key, bool&  value)
		{
			...
			if (RegQueryValueEx(m_hkey, key, NULL, &type, (unsigned char*)&data, &length) != ERROR_SUCCESS)
				return false;
		...
		bool Read(const char* key, char* string, DWORD length)
		{
			...
			DWORD type = REG_SZ;
			if (RegQueryValueEx(m_hkey, key, NULL, &type, (unsigned char*)string, &length) != ERROR_SUCCESS)
				return false;
		...
		bool Read(const char* key, void* value, DWORD length)
		{
			...
			if (RegQueryValueEx(m_hkey, key, NULL, &type, (unsigned char*)value, &length) != ERROR_SUCCESS)
				return false;
		...
		DWORD GetKeyLength(const char* key)
		{
			...
			DWORD type, length = 0;
			if (RegQueryValueEx(m_hkey, key, NULL, &type, NULL, &length) != ERROR_SUCCESS)
				return 0;
