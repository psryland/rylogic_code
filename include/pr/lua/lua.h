...
namespace pr
{
	namespace lua
	{
		...
		{
			PR_INFO(PR_DBG_LUA, pr::str::EnsureNewline(ToString(lua_state, 1)).c_str()); (void)lua_state;
			return 0;
		}
		...
		inline EResult::Type StepConsole(lua_State* lua_state, std::string input, std::string& syntax_error_msg)
		{
			// If the input says exit, return
			if (pr::str::EqualN(input, "exit", 4)) return EResult::Exit;
			...
			EResult::Type result = PushLuaChunk(lua_state, input, syntax_error_msg);
			switch (result)
			{
			...
			case EResult::Success:
				...
				if (lua_gettop(lua_state) != base && !lua_isnil(lua_state, -1))
				{
					...
					lua_pop(lua_state, 1);
					lua_pushstring(lua_state, pr::str::EnsureNewline(str).c_str());
					LuaPrint(lua_state);
				}
				break;
			}
