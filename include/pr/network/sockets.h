...
namespace pr
{
	namespace network
	{
		...
		inline bool SendTo(SOCKET socket, char const* host_ip, uint16 host_port, void const* data, size_t size, size_t max_packet_size, int timeout_ms = ~0)
		{
			...
			sockaddr_in addr = pr::network::GetAddress(host_ip, host_port);
		...
		protected:
			...
			template <typename ConnectionCB> size_t RemoveDeadConnections(ConnectionCB connect_cb)
			{
				...
				for (auto& client : m_clients)
				{
					...
					fd_set set = {};
					FD_SET(client, &set);
					auto timeout = pr::network::TimeVal(0);
					int result = ::select(0, &set, 0, 0, &timeout);
					if (result == 0)
		...
		public:
			...
			// 'connect_cb' should have the signature: void ConnectionCB(SOCKET socket, sockaddr_in const* client_addr);
			...
			{
				return pr::network::Send(client, data, size, m_max_packet_size, timeout_ms);
			}
			...
			bool Send(void const* data, size_t size, int timeout_ms = ~0)
			{
				...
				bool all_sent = true;
				for (auto& client : m_clients)
					all_sent &= pr::network::Send(client, data, size, m_max_packet_size, timeout_ms);
			...
			{
				return pr::network::SendTo(socket, host_ip, host_port, data, size, m_max_packet_size, timeout_ms);
			}
			...
			{
				bytes_read = 0;
				return pr::network::Recv(client, data, size, bytes_read, timeout_ms, flags);
			}
			...
			bool Recv(void* data, size_t size, size_t& bytes_read, int timeout_ms = 0, int flags = 0, SOCKET* out_client = nullptr)
			{
				...
				for (auto& client : m_clients)
				{
					...
					bytes_read = 0;
					if (pr::network::Recv(client, data, size, bytes_read, timeout_ms, flags) && bytes_read != 0)
					{
			...
			{
				return pr::network::RecvFrom(socket, host_ip, host_port, data, size, bytes_read, timeout_ms, flags);
			}
		};
		...
		public:
			...
			bool Send(void const* data, size_t size, int timeout_ms = ~0)
			{
				return pr::network::Send(m_socket, data, size, m_max_packet_size, timeout_ms);
			}
			bool Recv(void* data, size_t size, size_t& bytes_read, int timeout_ms = ~0, int flags = 0)
			{
				return pr::network::Recv(m_socket, data, size, bytes_read, timeout_ms, flags);
			}
			...
			{
				return pr::network::SendTo(m_port, host_ip, host_port, data, size, m_max_packet_size, timeout_ms);
			}
			bool RecvFrom(char const* host_ip, uint16 host_port, void* data, size_t size, size_t& bytes_read, int timeout_ms = ~0, int flags = 0)
			{
				return pr::network::RecvFrom(m_socket, host_ip, host_port, data, size, bytes_read, timeout_ms, flags);
			}
