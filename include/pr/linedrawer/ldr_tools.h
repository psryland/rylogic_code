	...
	namespace ldr
	{
		...
		public:
			...
			LdrMeasureUI(HWND parent, ReadPointCB read_point_cb, void* ctx, pr::Renderer& rdr)
				:Form(MakeFormParams<>().wndclass(RegisterWndClass<LdrMeasureUI>()).name("ldr-measure-ui").title(L"Measure Distances").wh(300, 110).style_ex('+',WS_EX_TOOLWINDOW).parent(parent).hide_on_close(true).pin_window(true))
				...
				,m_tb_values(pr::gui::TextBox::Params<>().parent(this_).name("tb-values").id(ID_TB_VALUES).xy(0, Top|BottomOf|ID_BTN_SET0).wh(Fill, Fill).margin(2,1,2,2).anchor(EAnchor::All).multiline(true))
		...
		public:
			...
			LdrAngleUI(HWND parent, ReadPointCB read_point_cb, void* ctx, pr::Renderer& rdr)
				:Form(MakeFormParams<>().wndclass(RegisterWndClass<LdrMeasureUI>()).name("ldr-angle-ui").title(L"Measure Angles").wh(144, 126).style_ex('+',WS_EX_TOOLWINDOW).parent(parent).hide_on_close(true).pin_window(true))
				,m_context_id(pr::GenerateGUID())
				,m_read_point_cb(read_point_cb)
				,m_read_point_ctx(ctx)
				,m_rdr(rdr)
				...
				,m_tb_values(pr::gui::TextBox::Params<>().parent(this_).name("tb-values").id(ID_TB_VALUES).xy(0, Top|BottomOf|ID_BTN_ORIG).wh(Fill, Fill).margin(2,1,2,2).anchor(EAnchor::All).multiline(true))
