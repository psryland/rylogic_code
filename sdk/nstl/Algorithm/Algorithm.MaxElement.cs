...
namespace NStl
{
    public static partial class Algorithm
	{
        ...
        public static FwdIt
            MaxElement<T, FwdIt>(FwdIt first, FwdIt last, IBinaryFunction<T, T, bool> less) where FwdIt : IForwardIterator<T>
        {
            ...
            while (!Equals(first.PreIncrement(), last))
                if (less.Execute(result.Value, first.Value))
        ...
        /// <returns>An iterator pointing to the maximum element of the range.</returns>
        public static FwdIt
            MaxElement<T, FwdIt>(FwdIt first, FwdIt last)
            where T : IComparable<T>
            where FwdIt : IForwardIterator<T>
        {
            return MaxElement(first, last, Compare.Less<T>());
        }
	}
}
