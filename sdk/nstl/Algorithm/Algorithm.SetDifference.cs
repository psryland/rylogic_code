...
namespace NStl
{
    public static partial class Algorithm
    {
        ...
        /// is less than another. The first version compares objects using <see cref="Compare.Less{T}"/>, and 
        ...
        /// </returns>
        public static OutIt
            SetDifference<T, OutIt>(IInputIterator<T> first1, IInputIterator<T> last1,
            ...
            where OutIt : IOutputIterator<T>
        {
            ...
            for (; !Equals(first1, last1) && !Equals(first2, last2); )
            {
                ...
                {
                    (dest.PostIncrement()).Value = first1.Value;
                    first1.PreIncrement();
                }
        ...
        /// SetDifference constructs a sorted range that is the set difference of the sorted 
        /// ranges [first1, last1) and [first2, last2). The return value is the end of the 
        /// sorted output range. The ordering criterion is <see cref="Compare.Less{T}"/>.
        /// </para>
        /// <para>
        /// In the simplest case, SetDifference performs the "difference" operation from 
        /// set theory: the output range contains a copy of every element that is contained 
        ...
        public static OutIt
            SetDifference<T, OutIt>(IInputIterator<T> first1, IInputIterator<T> last1,
                                    IInputIterator<T> first2, IInputIterator<T> last2, OutIt dest)
            where T : IComparable<T>
            where OutIt : IOutputIterator<T>
        {
            return SetDifference(first1, last1, first2, last2, dest, Compare.Less<T>());
        }
    }
}
