...
namespace NStl
{
    public static partial class Algorithm
	{
        ...
        /// </remarks>
        public static void
            MakeHeap<T>(IRandomAccessIterator<T> first, IRandomAccessIterator<T> last, IBinaryFunction<T, T, bool> comp)
        {
            ...
            for (; ; )
            {
                AdjustHeap(first, parent, length, (first.Add(parent)).Value, comp);
                if (parent == 0) return;
                --parent;
            }
        }
        ...
        /// </remarks>
        public static void
            MakeHeap<T>(IRandomAccessIterator<T> first, IRandomAccessIterator<T> last) where T : IComparable<T>
        {
            MakeHeap(first, last, Compare.Less<T>());
        }
	}
}
