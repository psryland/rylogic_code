...
namespace NStl
{
    public static partial class Algorithm
    {
        ...
        /// <param name="first">
        /// An <see cref="IBidirectionalIterator{T}"/> implemention
        ...
        /// </param>
        public static void
            Reverse<T>(IBidirectionalIterator<T> first, IBidirectionalIterator<T> last)
        {
            first = (IBidirectionalIterator<T>)first.Clone();
            last = (IBidirectionalIterator<T>)last.Clone();
            while (true)
                if (Equals(first, last) || Equals(first, last.PreDecrement()))
                    return;
                else
                    IterSwap(first.PostIncrement(), last);
        }
    }
}
