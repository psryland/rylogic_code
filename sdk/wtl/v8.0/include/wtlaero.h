...
public:
	...
	bool DrawPartText(HDC dc, int iPartID, int iStateID, LPCWSTR pStr, LPRECT prText, UINT uFormat, DTTOPTS &dto)
	{
		HRESULT hr = S_FALSE;
        if(IsTheming())
			if (IsSupported())
			...
			hr = CDCHandle(dc).DrawTextW(pStr, -1, prText, uFormat) != 0 ? S_OK : S_FALSE;

		return SUCCEEDED(hr);
	}

	 bool DrawPartText(HDC dc, int iPartID, int iStateID, LPCWSTR pStr, LPRECT prText, UINT uFormat, 
...
public:
		...
		DWORD dwFlags = DTT_COMPOSITED, int iGlowSize = 0)
	{
		...
		if (iLength > 0)
		{
			...
			GetWindowTextW(m_hWnd, sText, iLength);
