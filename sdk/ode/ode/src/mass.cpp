...
// Get mass properties from built-in Geom types
void dMassSetFromGeom (dMass *a, dGeomID g, dReal density)
{
	...
	default:
		if (dGeomIsSpace(g))
		{
			for (int i = 0, i_end = dSpaceGetNumGeoms((dSpaceID)g); i != i_end; ++i)
			{
				dMass m;
				dMassSetFromGeom(&m, dSpaceGetGeom((dSpaceID)g, i), density);
				dMassAdd(a, &m);
			}
			break;
		}
		dUASSERT(false, "Setting mass properties from this geom type is not implemented");
		return;
	case dSphereClass:
		dMassSetSphere(a, density, dGeomSphereGetRadius(g));
		break;
	case dBoxClass:
		{dVector3 dim;
		dGeomBoxGetLengths(g, dim);
		dMassSetBox(a, density, dim[0], dim[1], dim[2]);
		}break;
	case dCapsuleClass:
		{dReal radius, length;
		dGeomCapsuleGetParams(g, &radius, &length);
		dMassSetCapsule(a, density, 3, radius, length);
		}break;
	case dCylinderClass:
		{dReal radius, length;
		dGeomCapsuleGetParams(g, &radius, &length);
		dMassSetCylinder(a, density, 3, radius, length);
		}break;
  	}

	// Shift the mass by the geom offsets if present
	const dReal* pos = dGeomGetOffsetPosition(g);
	const dReal* rot = dGeomGetOffsetRotation(g);
	dMassRotate(a, rot);
	dMassTranslate(a, pos[0], pos[1], pos[2]);
}
