...
Sci::Position Editor::InsertText(const char* buffer, Sci::Position position, Sci::Position length, bool styled) {
	...
	if (styled)
	{
		constexpr Sci::Position BufSize = 4096;
		struct cell { char chr, sty; };
		char text[BufSize];
		char styl[BufSize];

		auto buf = reinterpret_cast<const cell*>(buffer);
		auto pos = position;
		if (length >= 0)
			length /= 2;

		// stop when we don't fill the local buffer
		for (int count = BufSize; count == BufSize; pos += count)
		{
			// Split 'buffer' into two separate buffers of text and styles.
			count = 0;
			if (length >= 0)
			{
				auto max_count = std::min(length - lengthInserted, BufSize);
...
sptr_t Editor::WndProc(Message iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case Message::AddStyledText: {
		...
		if (lParam == 0)
			return 0;
		const Sci::Position lengthInserted = InsertText(ConstCharPtrFromSPtr(lParam), CurrentPosition(), PositionFromUPtr(wParam), iMessage == Message::AddStyledText);
		SetEmptySelection(sel.MainCaret() + lengthInserted);
		return 0;
	}
	case Message::AppendText:
	case Message::AppendStyledText: {
		// Add text to the end of the document, without changing the selection
		if (lParam == 0)
			return 0;
		InsertText(ConstCharPtrFromSPtr(lParam), pdoc->Length(), PositionFromUPtr(wParam), iMessage == Message::AppendStyledText);
		return 0;
	}
	case Message::InsertText: 
	case Message::InsertStyledText: {
		...
		const Sci::Position lengthInserted = InsertText(sz, insertPos, -1, iMessage == Message::InsertStyledText);
