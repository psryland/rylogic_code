...
sptr_t Editor::WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam) {
	...
	case SCI_ADDSTYLEDTEXT: {
			...
			if (lParam == 0)
				return 0;
			const Sci::Position lengthInserted = InsertText(CharPtrFromSPtr(lParam), CurrentPosition(), static_cast<Sci::Position>(wParam), iMessage == SCI_ADDSTYLEDTEXT);
			SetEmptySelection(sel.MainCaret() + lengthInserted);
			return 0;
		}
	...
	case SCI_APPENDSTYLEDTEXT: {
			...
			if (lParam == 0)
				return 0;
			InsertText(CharPtrFromSPtr(lParam), pdoc->Length(), static_cast<Sci::Position>(wParam), iMessage == SCI_APPENDSTYLEDTEXT);
			return 0;
		}
	...
	case SCI_INSERTSTYLEDTEXT: {
			...
			const Sci::Position lengthInserted = InsertText(sz, insertPos, strlen(sz), iMessage == SCI_INSERTSTYLEDTEXT);
