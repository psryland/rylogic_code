...
namespace pr
{
	namespace rdr
	{
		...
		struct MdlSettings
		{
			...
			BoundingBox m_bbox;      // Model space bounding box
			...
			MdlSettings(VBufferDesc const& vb, IBufferDesc const& ib, BoundingBox const& bbox = pr::BBoxReset, char const* name = "")
				:m_vb(vb)
				,m_ib(ib)
				,m_bbox(bbox)
				,m_name(name)
			...
			MdlSettings(Vert const (&vert)[VSize], Indx const (&idxs)[ISize], BoundingBox const& bbox = pr::BBoxReset, char const* name = "")
			...
			MdlSettings(VCont const& vcont, ICont const& icont, BoundingBox const& bbox = BBoxReset, char const* name = "")
