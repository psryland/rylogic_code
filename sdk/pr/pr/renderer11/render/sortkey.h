...
//*********************************************
#pragma once
#ifndef PR_RDR_RENDER_sortkey_H
#define PR_RDR_RENDER_sortkey_H
...
namespace pr
{
	namespace rdr
	{
		...
		inline pr::rdr::SortKey MakeSortKey(Nugget const& nugget)
		{
			pr::rdr::DrawMethod const& meth = nugget.m_draw;
			bool alpha = false;
				
			SortKey key = 0;
			if (meth.m_shader)
			{
				key |= meth.m_shader->m_sort_id << sortkey::ShaderIdOfs;
				PR_ASSERT(PR_DBG_RDR, meth.m_shader->m_sort_id < sortkey::MaxShaderId, "shader sort id overflow");
			}
