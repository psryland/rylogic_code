...
namespace pr
{
	...
	struct Spline :pr::m4x4
	{
		...
		static Spline make(v4 const* spline)
		{
		...
		pr::m4x4 O2W(float time) const { return O2W(time, 2, pr::v4YAxis); }
		pr::m4x4 O2W(float time, int axis, v4 const& up) const
		{
	...
	{
		struct L { static float Len(pr::Spline const& s, float tol)
		{
	...
	inline float ClosestPoint_PointToSpline(pr::Spline const& spline, v4 const& pt, float initial_estimate, int iterations = 5)
	{
		...
		{
			pr::v4 S   = spline.Position(time);
			pr::v4 dS  = spline.Velocity(time);
	...
	struct RandSpline :pr::Spline
	{
		...
		,m_clock(0.0f)
		{
			m_next = pr::Spline::make(GenPoint(), GenPoint(), GenPoint(), GenPoint());
			Roll(); Roll();
		}
		void Reset()
		{
		...
		pr::v4 GenPoint()
		{
			return pr::Random3(m_bbox.Lower(), m_bbox.Upper(), 1.0f);
		}
		void Roll()
		{
			static_cast<pr::Spline&>(*this) = m_next;
			m_next.x = m_next.Point1();  m_next.y = m_next.Point1() + m_next.Forward1();
			m_next.z = GenPoint();       m_next.w = GenPoint();
		}
		void Adv(float dt)
		{
			m_clock += dt;
