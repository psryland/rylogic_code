...
namespace pr
{
	struct iv2
	{
		...
		int const&                 operator [] (uint i) const   { PR_ASSERT(PR_DBG_MATHS, i < 2, ""); return ToArray()[i]; }
		int&                       operator [] (uint i)         { PR_ASSERT(PR_DBG_MATHS, i < 2, ""); return ToArray()[i]; }
	...
	template <typename T> inline iv2& operator -= (iv2& lhs, T rhs) { lhs.x -= GetXi(rhs); lhs.y -= GetYi(rhs); return lhs; }
	template <typename T> inline iv2& operator *= (iv2& lhs, T rhs) { lhs.x *= GetXi(rhs); lhs.y *= GetYi(rhs); return lhs; }
	template <typename T> inline iv2& operator /= (iv2& lhs, T rhs) { PR_ASSERT(PR_DBG_MATHS, !IsZero4(rhs), ""); lhs.x /= GetXi(rhs); lhs.y /= GetYi(rhs); return lhs; }
	template <typename T> inline iv2& operator %= (iv2& lhs, T rhs) { PR_ASSERT(PR_DBG_MATHS, !IsZero4(rhs), ""); lhs.x %= GetXi(rhs); lhs.y %= GetYi(rhs); return lhs; }
	...
	inline iv2 operator - (int lhs, iv2 const& rhs)                     { iv2 v = rhs; return v -= lhs; }
	inline iv2 operator * (int lhs, iv2 const& rhs)                     { iv2 v = rhs; return v *= lhs; }
	inline iv2 operator / (int lhs, iv2 const& rhs)                     { PR_ASSERT(PR_DBG_MATHS, All4(rhs,maths::NonZero<int>), ""); return iv2::make(lhs / rhs.x, lhs / rhs.y); }
	inline iv2 operator % (int lhs, iv2 const& rhs)                     { PR_ASSERT(PR_DBG_MATHS, All4(rhs,maths::NonZero<int>), ""); return iv2::make(lhs % rhs.x, lhs % rhs.y); }
