...
namespace pr
{
	inline v2 Random2N(Rnd& rnd)                                                        { v2 v; float len; do { v = v2::make(rnd.fltc(0.0f,1.0f), rnd.fltc(0.0f,1.0f)); len = Length2Sq(v); } while (len > 1.0f || len == 0.0f); return Normalise2(v); }
	...
	inline v2 Random2(Rnd& rnd, float min_length, float max_length)                     { return rnd.fltr(min_length, max_length) * Random2N(rnd); }
	...
	inline v3 Random3N(Rnd& rnd)
	{
		...
		do
		{
			v = v3::make(rnd.fltc(0.0f,1.0f), rnd.fltc(0.0f,1.0f), rnd.fltc(0.0f,1.0f));
			len = Length3Sq(v);
		}
	...
	inline v3 Random3(Rnd& rnd, float min_length, float max_length)                     { return rnd.fltr(min_length, max_length) * Random3N(rnd); }
	...
	inline v4 Random4N(Rnd& rnd)
	{
		...
		do
		{
			v = v4::make(rnd.fltc(0.0f, 1.0f), rnd.fltc(0.0f, 1.0f), rnd.fltc(0.0f, 1.0f), rnd.fltc(0.0f, 1.0f));
			len = Length4Sq(v);
		}
		while (len > 1.0f || len == 0.0f);
		return v /= Sqrt(len);
	...
	inline v4 Random4(Rnd& rnd, float min_length, float max_length)                     { return rnd.fltr(min_length, max_length) * Random4N(rnd); }
	inline v4 Random4(          float min_length, float max_length)                     { return Random4(rand::Rand(), min_length, max_length); }
	inline v4 Random4(Rnd& rnd, v4 const& vmin, v4 const& vmax)                         { return v4::make(rnd.fltr(vmin.x,vmax.x), rnd.fltr(vmin.y,vmax.y), rnd.fltr(vmin.z,vmax.z), rnd.fltr(vmin.w,vmax.w)); }
	...
	inline m2x2 Random2x2(Rnd& rnd, float min_angle, float max_angle)                   { return m2x2::make(rnd.fltr(min_angle, max_angle)); }
	...
	inline m4x4 Random4x4(Rnd& rnd, float min_value, float max_value)
	{
		...
		m.w.set(rnd.fltr(min_value, max_value), rnd.fltr(min_value, max_value), rnd.fltr(min_value, max_value), rnd.fltr(min_value, max_value));
		return m;
	}
	...
	inline Quat RandomQ(Rnd& rnd, v4 const& axis, float min_angle, float max_angle) { return Quat::make(axis, rnd.fltr(min_angle, max_angle)); }
	inline Quat RandomQ(          v4 const& axis, float min_angle, float max_angle) { return RandomQ(rand::Rand(), axis, min_angle, max_angle); }
	inline Quat RandomQ(Rnd& rnd, float min_angle, float max_angle)                 { return Quat::make(Random3N(rnd, 0.0f), rnd.fltr(min_angle, max_angle)); }
	inline Quat RandomQ(          float min_angle, float max_angle)                 { return RandomQ(rand::Rand(), min_angle, max_angle); }
	inline Quat RandomQ(Rnd& rnd)                                                   { return Quat::make(Random3N(rnd, 0.0f), rnd.fltr(0.0f, maths::tau)); }
