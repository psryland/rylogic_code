...
namespace pr
{
	namespace ph
	{
		...
		struct ShapePolytopeHelper
		{
			...
			ShapePolytope& set(v4 const* verts, std::size_t num_verts, m4x4 const& shape_to_model, MaterialId material_id, uint flags);

			// Use an array of verts and faces to create a polytope. Verts and faces must be convex
			ShapePolytope& set(v4 const* verts, std::size_t num_verts, ShapePolyFace const* faces, std::size_t num_faces, m4x4 const& shape_to_model, MaterialId material_id, uint flags);
		};
	}
}
