...
namespace WTL
{
	// Create one of these to register the scintilla window class
	struct InitScintilla
	{
		InitScintilla(HINSTANCE hinstance = ModuleHelper::GetModuleInstance())
		{
			// ++/-- operators on atomics are read-modify-write so there's isn't a race here
			if (refcount()++ == 0)
			{
				if (Scintilla_RegisterClasses(hinstance) == 0)
					throw std::exception("Scintilla initialisation failure");
			}
		}
		~InitScintilla()
		{
			if (--refcount() == 0)
			{
				if (Scintilla_ReleaseResources() == 0)
					throw std::exception("Scintilla shutdown failure");
			}
		}
	private:
		static std::atomic_int& refcount() { static std::atomic_int rc; return rc; }
	};

	// Wrapper for the scintilla edit control
	class ScintillaCtrl :public CWindow
	{
		typedef unsigned int uint;

		#pragma region Helpers
		template <typename Ret, typename WPRM, typename LPRM> Ret SndMsg(int msg, WPRM wparam, LPRM lparam) const
		{
			auto res = ::SendMessage(m_hWnd, UINT(msg), (WPARAM)wparam, (LPARAM)lparam);
			return (Ret)(res);
		}
