...
namespace pr.extn
{
	...
	[TestFixture] internal partial class UnitTests
	{
		...
		[Test] public static void TestXmlAs()
		{
			...
			Assert.NotNull(root); if (root == null) return;
			...
			Assert.AreEqual(3         ,root.Element("a").As((n)=>{return 2+n.As<int>();}));
			...
			ints.Clear();
			root.Element("e").As(ints, "i", (lhs,rhs)=>{return lhs == rhs;}); Assert.AreEqual(4, ints.Count);
			for (int i = 0; i != 4; ++i) Assert.AreEqual(i, ints[i]);

			ints.Clear();
			root.Element("e").As(ints, "i", (n)=>{return 2+n.As<int>();}); Assert.AreEqual(5, ints.Count);
		...
		[Test] public static void TextXmlAdd()
		{
			...
			Elem[] elems = Util.New(5, (i)=>{return new Elem(i);});
			string s;
