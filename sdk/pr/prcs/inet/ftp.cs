...
namespace pr.inet
{
	...
	/// FTPS = ftp explicit SSL. Note FTPS != SFTP (ssh ftp)
	/// </summary>
	public class FTPConnection :IDisposable
	{
		...
		{
			SslStream stream = new SslStream(src);
		...
		public void Upload(string local_filepath, string remote_filename, ResumeBehaviour resume = ResumeBehaviour.DontResume)
		{
			...
			// Upload the file
			using (FileStream input = new FileStream(local_filepath, FileMode.Open, FileAccess.Read))
			{
