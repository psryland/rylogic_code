...
namespace SolarHotWater
{
	public sealed class Model :IDisposable, INotifyPropertyChanged
	{
		...
		private void RunMonitor()
		{
			...
			// Find the set of consumers that can be turned on given the current solar output

			// If within an "On" period as indicated by the timers on the switch, do nothing

			// If currently "On" and the last "on" event wasn't generated by this app, do nothing

			// For all consumers, turn off or on as appropriate (if not within the cooldown)
		...
		private void PopulateConsumerDevices(IList<Consumer> consumers)
		{
			...
			foreach (var consumer in consumers)
				consumer.EweDevice = devices.TryGetValue(consumer.SwitchName, out var sw) ? sw : null;
		}
