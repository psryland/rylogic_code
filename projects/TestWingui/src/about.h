...
namespace pr
{
	namespace gui
	{
		...
		struct About :Form
		{
			...
			About()
				:Form(MakeDlgParams<>().name("about").id(IDD_ABOUTBOX).start_pos(EStartPosition::CentreParent))
		...
		struct About2 :Form
		{
			...
			About2()
				:Form(MakeDlgParams<>().name("about2").title(L"About2 - TestWinGUI").wh(163,62).dlu().start_pos(EStartPosition::CentreParent).wndclass(RegisterWndClass<About2>()))
