...
{
	float4 skinned_vert = float4(0, 0, 0, 0);
	skinned_vert += mul(norm, pose[influence.m_bones.x].m) * influence.m_weights.x;
	skinned_vert += mul(norm, pose[influence.m_bones.y].m) * influence.m_weights.y;
	skinned_vert += mul(norm, pose[influence.m_bones.z].m) * influence.m_weights.z;
	skinned_vert += mul(norm, pose[influence.m_bones.w].m) * influence.m_weights.w;
	return skinned_vert;
}
