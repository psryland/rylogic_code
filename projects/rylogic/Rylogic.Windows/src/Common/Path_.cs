...
namespace Rylogic.Common.Windows
{
	public static class Path_
	{
		...
		public static bool IsSamePath(string lhs, string rhs)
		{
			...
			{
				return Kernel32.CreateFile(path, Win32.EFileAccess.NEITHER, Win32.EFileShare.READ | Win32.EFileShare.WRITE, IntPtr.Zero, Win32.EFileCreation.OPEN_EXISTING, Win32.EFileFlag.BACKUP_SEMANTICS, IntPtr.Zero);
			}
			static Win32.BY_HANDLE_FILE_INFORMATION? GetFileInfo(SafeFileHandle handle)
			{
				return handle != null && Kernel32.GetFileInformationByHandle(handle.DangerousGetHandle(), out var object_file_info)
