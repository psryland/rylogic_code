	...
	void GpuSortAndSweep::EnumOverlappingPairs(std::function<void(pr::physics::RigidBody const&, pr::physics::RigidBody const&)> cb) const
	{
		...
		for (int i = 0; i != endpoint_count; ++i)
		{
			...
			else
			{
				...
				auto& bb_new = m_bboxes[body_idx];
				auto new_lo_y = bb_new.Centre()[axis_y] - bb_new.Radius()[axis_y];
				auto new_hi_y = bb_new.Centre()[axis_y] + bb_new.Radius()[axis_y];
				auto new_lo_z = bb_new.Centre()[axis_z] - bb_new.Radius()[axis_z];
				auto new_hi_z = bb_new.Centre()[axis_z] + bb_new.Radius()[axis_z];

				for (auto active_idx : active)
				{
					auto& bb_act = m_bboxes[active_idx];

					// Y-axis overlap test
					auto act_lo_y = bb_act.Centre()[axis_y] - bb_act.Radius()[axis_y];
					auto act_hi_y = bb_act.Centre()[axis_y] + bb_act.Radius()[axis_y];
					if (new_lo_y > act_hi_y || new_hi_y < act_lo_y)
						continue;

					// Z-axis overlap test
					auto act_lo_z = bb_act.Centre()[axis_z] - bb_act.Radius()[axis_z];
					auto act_hi_z = bb_act.Centre()[axis_z] + bb_act.Radius()[axis_z];
					if (new_lo_z > act_hi_z || new_hi_z < act_lo_z)
						continue;
