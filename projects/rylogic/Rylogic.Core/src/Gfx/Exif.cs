...
namespace Rylogic.Gfx
{
	...
	public static class Exif
	{
		...
		private static IExifDataInternal Parse(Stream src, IExifDataInternal data, bool load_thumbnail)
		{
			...
			using (var br = new BinaryReaderEx(src, Encoding.UTF8, true))
			{
		...
		private class BinaryReaderEx :IDisposable
		{
			...
			public BinaryReaderEx(Stream input) :this(input, Encoding.UTF8, true) {}
			public BinaryReaderEx(Stream input, Encoding encoding, bool leave_open) { m_br = new BinaryReader(input, encoding, leave_open); }
		...
		private class BinaryWriterEx :IDisposable
		{
			...
			public BinaryWriterEx(Stream input, Encoding encoding) { m_bw = new BinaryWriter(input, encoding, true); }
...
namespace Rylogic.UnitTests
{
	...
	[TestFixture] public class TestExif
	{
		...
		{
			SrcImageWith    = Path.Combine(UnitTest.ResourcePath, "exif_image_with.jpg");
			SrcImageWithout = Path.Combine(UnitTest.ResourcePath, "exif_image_without.jpg");
		}
