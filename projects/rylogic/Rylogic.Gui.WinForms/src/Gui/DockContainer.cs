...
namespace Rylogic.Gui.WinForms
{
	...
	public class DockContainer :ContainerControl ,ITreeHost
	{
		...
		public ToolStripMenuItem WindowsMenu(string menu_name = "Windows")
		{
			...
			var filter = Utility.Util.FileDialogFilter("Layout Files","*.xml");
		...
		public class DockSizeData
		{
			...
			public DockSizeData(XElement node)
				: this()
			{
				Left   = node.Element(nameof(Left))  .As<float>(Left);
				Top    = node.Element(nameof(Top))   .As<float>(Top);
				Right  = node.Element(nameof(Right)) .As<float>(Right);
				Bottom = node.Element(nameof(Bottom)).As<float>(Bottom);
			}
