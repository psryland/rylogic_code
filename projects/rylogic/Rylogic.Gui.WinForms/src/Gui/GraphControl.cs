	...
	public class GraphControl :UserControl
	{
		...
		public class RdrOptions :INotifyPropertyChanged
		{
			...
			public RdrOptions(XElement node) :this()
			{
				...
				PlotBkColour    = node.Element(XmlTag.PlotBkColour   ).As<Color>(PlotBkColour);
				AxisColour      = node.Element(XmlTag.AxisColour     ).As<Color>(AxisColour);
				GridColour      = node.Element(XmlTag.GridColour     ).As<Color>(GridColour);
				TitleColour     = node.Element(XmlTag.TitleColour    ).As<Color>(TitleColour);
				TitleFont       = node.Element(XmlTag.TitleFont      ).As<Font>(TitleFont);
				TitleTransform  = node.Element(XmlTag.TitleTransform ).As<Matrix>(TitleTransform);
				Margin          = node.Element(XmlTag.Margin         ).As<Padding>(Margin);
				NoteFont        = node.Element(XmlTag.NoteFont       ).As<Font>(NoteFont);
				SelectionColour = node.Element(XmlTag.SelectionColour).As<Color>(SelectionColour);
				XAxis           = node.Element(XmlTag.XAxis          ).As<Axis>(XAxis);
				YAxis           = node.Element(XmlTag.YAxis          ).As<Axis>(YAxis);
			}
			...
			public class Axis
			{
				...
				{
					LabelFont      = node.Element(XmlTag.LabelFont     ).As<Font>(LabelFont     );
					TickFont       = node.Element(XmlTag.TickFont      ).As<Font>(TickFont      );
					AxisColour     = node.Element(XmlTag.AxisColour    ).As<Color>(AxisColour    );
					LabelColour    = node.Element(XmlTag.LabelColour   ).As<Color>(LabelColour   );
					TickColour     = node.Element(XmlTag.TickColour    ).As<Color>(TickColour    );
					TickLength     = node.Element(XmlTag.TickLength    ).As<int>(TickLength    );
					MinTickSize    = node.Element(XmlTag.MinTickSize   ).As<float>(MinTickSize   );
					DrawTickMarks  = node.Element(XmlTag.DrawTickMarks ).As<bool>(DrawTickMarks );
					DrawTickLabels = node.Element(XmlTag.DrawTickLabels).As<bool>(DrawTickLabels);
					LabelTransform = node.Element(XmlTag.LabelTransform).As<Matrix>(LabelTransform);
					PixelsPerTick  = node.Element(XmlTag.PixelsPerTick ).As<double>(PixelsPerTick );
				}
				public XElement ToXml(XElement node)
				{
