...
namespace Rylogic.Gui.WinForms
{
	...
	public sealed class CredentialsUI
	{
		...
		public CredentialsUI(string target) : this(target, null)
		{ }
		...
		public CredentialsUI(string target, string caption) : this(target, caption, null)
		{ }
		...
		public CredentialsUI(string target, string caption, string message) : this(target, caption, message, null)
		{ }
		...
		public CredentialsUI(string target, string caption, string message, Image banner)
		{
			Target = target;
			Caption = caption;
			Message = message;
		...
		/// <summary>Gets or sets if the dialog will be shown even if the credentials can be returned from an existing credential in the credential manager.</summary>
		...
		/// <summary>Gets or sets if the save check box is displayed.</summary>
		...
		/// <summary>Shows the credentials dialog with the specified save check box status.</summary>
		/// <param name="saveChecked">True if the save check box is checked.</param>
		/// <returns>Returns a DialogResult indicating the user action.</returns>
		...
		/// <summary>Shows the credentials dialog with the specified name, password and save check box status.</summary>
		/// <param name="name">The name for the credentials.</param>
		/// <param name="password">The password for the credentials.</param>
		/// <param name="saveChecked">True if the save check box is checked.</param>
		/// <returns>Returns a DialogResult indicating the user action.</returns>
		...
		/// <summary>Shows the credentials dialog with the specified owner and save check box status.</summary>
		/// <param name="owner">The System.Windows.Forms.IWin32Window the dialog will display in front of.</param>
		/// <param name="saveChecked">True if the save check box is checked.</param>
		/// <returns>Returns a DialogResult indicating the user action.</returns>
		...
		/// <summary>Shows the credentials dialog with the specified owner, name, password and save check box status.</summary>
		/// <param name="owner">The System.Windows.Forms.IWin32Window the dialog will display in front of.</param>
		/// <param name="name">The name for the credentials.</param>
		/// <param name="password">The password for the credentials.</param>
		/// <param name="saveChecked">True if the save check box is checked.</param>
		...
		public void Confirm(bool value)
		{
			...
			{
			default: throw new ApplicationException("Credential confirmation failed.");
			case CREDUI.ReturnCodes.NO_ERROR: break;
			case CREDUI.ReturnCodes.ERROR_INVALID_PARAMETER: break; // for some reason, this is encountered when credentials are overwritten
			}
		}
		...
		private DialogResult ShowDialog(IWin32Window owner)
		{
			...
			// clean up resources
			if (Banner != null) Win32.DeleteObject(info.hbmBanner);
		...
		private CREDUI.INFO GetInfo(IWin32Window owner)
		{
			...
			if (Banner != null) info.hbmBanner = new Bitmap(Banner, ValidBannerWidth, ValidBannerHeight).GetHbitmap(); 
		...
		private CREDUI.FLAGS GetFlags()
		{
			...
			if (!Persist           ) flags = flags | CREDUI.FLAGS.DO_NOT_PERSIST;
			else
			{
		...
		private static DialogResult GetDialogResult(CREDUI.ReturnCodes code)
		{
			switch (code)
			{
			default: throw new ApplicationException("Unknown credential result encountered.");
			case CREDUI.ReturnCodes.NO_ERROR:                    return DialogResult.OK;
			case CREDUI.ReturnCodes.ERROR_CANCELLED:             return DialogResult.Cancel;
			case CREDUI.ReturnCodes.ERROR_NO_SUCH_LOGON_SESSION: throw new ApplicationException("No such log-on session.");
			case CREDUI.ReturnCodes.ERROR_NOT_FOUND:             throw new ApplicationException("Not found.");
			case CREDUI.ReturnCodes.ERROR_INVALID_ACCOUNT_NAME:  throw new ApplicationException("Invalid account name.");
			case CREDUI.ReturnCodes.ERROR_INSUFFICIENT_BUFFER:   throw new ApplicationException("Insufficient buffer.");
			case CREDUI.ReturnCodes.ERROR_INVALID_PARAMETER:     throw new ApplicationException("Invalid parameter.");
			case CREDUI.ReturnCodes.ERROR_INVALID_FLAGS:         throw new ApplicationException("Invalid flags.");
			}
		}
	}
