...
namespace pr.maths
{
	...
	public class SlidingDFT
	{
		...
		public void Reset(int window_size, double sampling_frequency, bool inverse = false)
		{
			...
			var k = (!inverse ? +Maths.Tau : -Maths.Tau) * new Complex(0,1) / (double)window_size;
		...
		public double Power(double freq)
		{
			...
			return Maths.Lerp(a, b, fidx - idx);
		}
...
namespace pr.unittests
{
	...
	[TestFixture] public class TestFFT
	{
		[Test] public void SlidingDFT()
		{
			...
			{
				signal[i] =
					Math.Sin(Maths.Tau * freq0 * i / SampFreq) +
					Math.Sin(Maths.Tau * freq1 * i / SampFreq) +
					Math.Sin(Maths.Tau * freq2 * i / SampFreq) +
					Math.Sin(Maths.Tau * freq3 * i / SampFreq) +
					Math.Sin(Maths.Tau * freq4 * i / SampFreq);
			}
			...
			{// Sliding Window DFT
				...
				{// Output the frequency response
					...
					for (double x = freq_range.Beg; x < freq_range.End; x += 0.1)
						s_out.AppendLine("{0}, {1}".Fmt(x, dft.Power(x)));
