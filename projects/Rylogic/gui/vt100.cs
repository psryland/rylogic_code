	...
	public class VT100_buf
	{
		...
		private void ParseOutput(string text)
		{
			...
			for (;e != eend;)
			{
				...
				{
					if (s != e) Write(text.Substring(s, e - s));
					m_seq.Append(c);
					ParseEscapeSeq();
					++e;
					s = e;
				...
				else if (char.IsControl(c)) // ignore non printable text
				{
					...
					if (c == '\t') Write("".PadRight(Settings.TabSize - (m_out.pos.X % Settings.TabSize), ' '));
					++e;
					s = e;
				}
			}
			...
			if (s != eend) Write(text.Substring(s, eend - s));
		}
		...
		{
			var code = m_seq.Back(); m_seq.Length--;
			var field = m_seq.ToString(2);
			switch (code)
			{
		...
		/// to an array of integers. Returns at least 'N' results</summary>
		private static int[] Params(int N, string param_string)
		{
			string[] str_parms = param_string.Split(';');
			var parms = new int[Math.Max(str_parms.Length, N)];
			for (int i = 0; i != str_parms.Length; ++i)
				int.TryParse(str_parms[i], out parms[i]);
			return parms;
		}
		...
		private void Write(string str, int count)
		{
			System.Diagnostics.Debug.Assert(m_out.pos.X >= 0 && m_out.pos.X < Settings.TerminalWidth, "Caret outside screen buffer");
			System.Diagnostics.Debug.Assert(m_out.pos.Y >= 0 && m_out.pos.Y < Settings.TerminalHeight, "Caret outside screen buffer");

			// Get the line and ensure it's large enough
			var line = LineAt(m_out.pos.Y);
			if (line.size <= m_out.pos.X)
				line.resize(Math.Min(m_out.pos.X, Settings.TerminalWidth), ' ', m_out.style);

			// Write the string
			var len = Math.Min(Settings.TerminalWidth - m_out.pos.X, count);
			line.write(m_out.pos.X, str, len, m_out.style);
		}
		void Write(string str)
		{
			Write(str, Math.Min(str.Length, Settings.TerminalWidth - m_out.pos.X));
		}
