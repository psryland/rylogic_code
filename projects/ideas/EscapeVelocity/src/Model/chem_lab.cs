...
namespace EscapeVelocity
{
	...
	public class ChemLab
	{
		...
		{
			m_consts = consts;
			KnownElements = new List<IElementKnown>();
			KnownMaterials = new List<Compound>();
		...
		private void RaiseDiscovery(Element element) => Discovery?.Invoke(this, new DiscoveryEventArg { Element = element });
		private void RaiseDiscovery(Compound compound) => Discovery?.Invoke(this, new DiscoveryEventArg { Compound = compound });

		/// <summary>The known elements, sorted by atomic number where known, otherwise alphabetically</summary>
		public List<IElementKnown> KnownElements { get; }

		/// <summary>The known Compound, sorted into the display order</summary>
		public List<Compound> KnownMaterials { get; }
		...
		private void UpdateKnown()
		{
			...
			KnownElements.Clear();
			KnownElements.AddRange(m_elements.Where(x => (x.KnownProperties & EElemProp.Existence) != 0));
			...
			KnownMaterials.AddRange(m_compounds.Where(x => x.Discovered));
