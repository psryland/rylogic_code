...
namespace Rylogic.Gui.WPF
{
	...
	public partial class ChartControl
	{
		...
		public class MouseOps
		{
			...
			private readonly Dictionary<MouseButton, MouseOp> m_pending;

			public MouseOps()
			{
				m_pending = Enum<MouseButton>.Values.ToDictionary(k => k, k => (MouseOp)null);
			}
		...
		public abstract class MouseOp : IDisposable
		{
			...
			public PointF m_grab_client; // The client space location of where the chart was "grabbed" (note: ChartControl, not ChartPanel space)
			...
			public bool IsClick(PointF location)
			{
		...
		public class MouseOpDefaultLButton : MouseOp
		{
			...
			{
				var location = e.GetPosition(m_chart).ToPointF();
				...
				if (m_chart.Options.NavigationMode == ENavMode.Scene3D && m_hit_axis == EAxis.None)
					m_chart.Scene.Window.MouseNavigate(location, e.ToMouseBtns(Keyboard.Modifiers), View3d.ENavOp.Rotate, true);
			...
			public override void MouseMove(MouseEventArgs e)
			{
				...
				if (drag_selected)
				{
					...
					var delta = Point_.Subtract(m_chart.ClientToChart(location), m_grab_chart);
				...
				else if (m_chart.Options.NavigationMode == ENavMode.Chart2D)
				{
					if (m_chart.AreaSelectMode != EAreaSelectMode.Disabled)
					{
						...
						var selection_area = BRect.FromBounds(m_grab_chart, m_chart.ClientToChart(location));
				...
				else if (m_chart.Options.NavigationMode == ENavMode.Scene3D)
				{
					...
					m_chart.Scene.Window.MouseNavigate(location, e.ToMouseBtns(Keyboard.Modifiers), View3d.ENavOp.Rotate, false);
				}
				//m_chart.Invalidate();
				//m_chart.Update();
			}
			public override void MouseUp(MouseButtonEventArgs e)
			{
				...
				else
				{
					...
					else if (m_chart.Options.NavigationMode == ENavMode.Chart2D)
					{
						...
						{
							var selection_area = BBox.From(new v4(m_grab_chart, 0, 1f), new v4(m_chart.ClientToChart(location), 0f, 1f));
		...
		public class MouseOpDefaultRButton : MouseOp
		{
			...
			public override void MouseDown(MouseButtonEventArgs e)
			{
				...
				var loc = Gui_.MapPoint(m_chart, m_chart.Scene, location.ToSysWinPoint()).ToPointF();
				m_chart.Scene.Window.MouseNavigate(loc, e.ToMouseBtns(Keyboard.Modifiers), View3d.ENavOp.Translate, true);
			}
			public override void MouseMove(MouseEventArgs e)
			{
				var location = e.GetPosition(m_chart).ToPointF();
				...
				m_chart.Scene.Window.MouseNavigate(drop_loc, e.ToMouseBtns(Keyboard.Modifiers), View3d.ENavOp.Translate, false);
			...
			public override void MouseUp(MouseButtonEventArgs e)
			{
				...
				if (IsClick(location))
				{
					...
					if (!args.Handled)
					{
						...
						{
							var location1 = args.GetPosition(m_chart);
				...
				else
				{
					...
					m_chart.Scene.Window.MouseNavigate(drop_loc, e.ToMouseBtns(Keyboard.Modifiers), View3d.ENavOp.None, true);
					m_chart.SetRangeFromCamera();
					//m_chart.Invalidate();
				}
			}
