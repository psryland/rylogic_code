...
namespace Tradee
{
	public class EditEmaUI :ToolForm
	{
		private readonly EmaIndicator m_ema;
		...
		private void SetupUI()
		{
			...
			m_spinner_window_size.Maximum = EmaSettings.MaxWindowSize;
			m_spinner_window_size.Value = Maths.Clamp(m_ema.Settings.WindowSize, m_spinner_window_size.Minimum, m_spinner_window_size.Maximum);
			m_spinner_window_size.ValueChanged += (s,a) =>
			{
				m_ema.Settings.WindowSize = (int)m_spinner_window_size.Value;
			};

			// Width
			m_spinner_width.Minimum = 0;
			m_spinner_width.Maximum = 50;
			m_spinner_width.Value = Maths.Clamp(m_ema.Settings.Width, m_spinner_width.Minimum, m_spinner_width.Maximum);
			m_spinner_width.ValueChanged += (s,a) =>
			{
				m_ema.Settings.Width = (int)m_spinner_width.Value;
			};

			// EMA Colour
			m_colour_ema.Colour = m_ema.Settings.ColourEMA;
			m_colour_ema.ColourChanged += (s,a) =>
			{
				m_ema.Settings.ColourEMA = m_colour_ema.Colour;
			};

			// Bollinger Bands
			m_spinner_bollinger_band_stddev.Minimum = 0;
			m_spinner_bollinger_band_stddev.Maximum = 5;
			m_spinner_bollinger_band_stddev.Value = Maths.Clamp((decimal)m_ema.Settings.BollingerBands, m_spinner_bollinger_band_stddev.Minimum, m_spinner_bollinger_band_stddev.Maximum);
			m_spinner_bollinger_band_stddev.ValueChanged += (s,a) =>
			{
				m_ema.Settings.BollingerBands = (float)m_spinner_bollinger_band_stddev.Value;
			};

			// Bollinger Colour
			m_colour_bollinger.Colour = m_ema.Settings.ColourBollingerBands;
			m_colour_bollinger.ColourChanged += (s,a) =>
			{
				m_ema.Settings.ColourBollingerBands = m_colour_bollinger.Colour;
			};

			// Delete button
			m_btn_delete.Click += (s,a) =>
			{
				Chart.Indicators.Remove(m_ema);
				Close();
			};
		}
