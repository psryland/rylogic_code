...
namespace Tradee
{
	...
	public class SupportResist :IDisposable
	{
		public SupportResist(MainModel model)
		{
			Model = model;
			SnRLevels = new InstrumentSnRLevels();
		}
		...
		public MainModel Model
		{
			...
			set
			{
				...
				if (m_model != null)
				{
					m_model.MarketData.DataChanged -= HandleDataChanged;
				}
				m_model = value;
				if (m_model != null)
				{
		...
		{
			[DebuggerStepThrough] get { return Model.MarketData; }
		}

		/// <summary>The support and resistance levels for instruments</summary>
		public InstrumentSnRLevels SnRLevels
		{
			get;
			private set;
		}
		public class InstrumentSnRLevels :Dictionary<string, SnRLevels> {}

		/// <summary>Handle new market data arriving</summary>
		private void HandleDataChanged(object sender, DataEventArgs args)
		{
			SnRLevels snr;
			if (args.Instrument != null && SnRLevels.TryGetValue(args.SymbolCode, out snr))
			{
				snr.HandleDataChanged(args);
			}
		}
	}

	/// <summary>Sorted list of price levels and number of orders at that level</summary>
	public class SnRLevels
	{
		...
		internal void HandleDataChanged(DataEventArgs args)
		{
