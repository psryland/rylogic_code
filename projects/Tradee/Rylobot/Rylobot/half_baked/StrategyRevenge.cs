...
namespace Rylobot
{
	public class StrategyRevenge :Strategy
	{
		...
		private void SyncTrades()
		{
			...
			Trades.RemoveIf(x => x.Id == null || !live_positions.Contains(x.Id.Value));
		}
		...
		private Trade FindEntry()
		{
			...
			{// Look for some confirming signals
				...
				var grad100 = ema100.Result.FirstDerivative() / Instrument.MCS_50;
				...
				var grad14 = ema14.Result.FirstDerivative() / Instrument.MCS_50;
				if (Math.Abs(grad14) < ema14_threshold_gradient || Math.Sign(grad14) != sign)
					return null;
				
				// Is the price within the MCS of the long period EMA?
				if (Math.Abs(curr_price - ema100.Result.LastValue) > Instrument.MCS_50)
					return null;
