...
void pr::rdr::Video::CreateFromFile(D3DPtr<IDirect3DDevice9>& d3d_device, char const* filepath)
{
	Free();
	
	// Create the filter graph
	D3DPtr<IGraphBuilder> graph_builder;
	if (Failed(CoCreateInstance(CLSID_FilterGraph, 0, CLSCTX_INPROC_SERVER, IID_IGraphBuilder, (void**)&graph_builder.m_ptr)))
		throw RdrException(EResult::CreateInterfaceFailed, "Failed to create an instance of the directshow graph builder interface");
		
	// Create a VMR9 filter
	D3DPtr<IBaseFilter> vmr9;
	Throw(CoCreateInstance(CLSID_VideoMixingRenderer9, 0, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void**)&vmr9.m_ptr));
	...
	Throw(vmr9->QueryInterface(IID_IVMRFilterConfig9, (void**)&config.m_ptr));
	Throw(config->SetNumberOfStreams(1));
	Throw(config->SetRenderingMode(VMR9Mode_Renderless));
	
	// Get the surface allocator notify interface
	D3DPtr<IVMRSurfaceAllocatorNotify9> surface_alloc_notify;
	Throw(vmr9->QueryInterface(IID_IVMRSurfaceAllocatorNotify9, (void**)&surface_alloc_notify.m_ptr));
	Throw(surface_alloc_notify->SetD3DDevice(d3d_device.m_ptr, pr::rdr::GetMonitor(d3d_device)));
	...
	m_alloc_pres = new AllocPres(*this, d3d_device);
	...
	Throw(graph_builder->QueryInterface(IID_IMediaControl  ,(void**)&m_media_control.m_ptr));
	Throw(graph_builder->QueryInterface(IID_IMediaEvent    ,(void**)&m_media_event.m_ptr));
	Throw(graph_builder->QueryInterface(IID_IMediaPosition ,(void**)&m_media_position.m_ptr));
...
void pr::rdr::Video::Stop()
{
	...
	Throw(m_media_control->Stop());
}

// Device lost/restored
void pr::rdr::Video::OnEvent(pr::rdr::Evt_DeviceLost const&)
{
	// There is a bug in VMR-9 which causes an internal texture not to be released on device lost.
	// Because of this we need to tear down the whole graph
	Free();
}
void pr::rdr::Video::OnEvent(pr::rdr::Evt_DeviceRestored const& e)
{
	D3DPtr<IDirect3DDevice9> d3d_device = e.m_d3d_device; // because of const correctness
	CreateFromFile(d3d_device, m_filepath.c_str());
}
...
//	if (hwnd != 0)
//	{
//		Throw(video_window->put_Owner(OAHWND(hwnd)));                                    // Parent the window
//		Throw(video_window->put_WindowStyle(WS_CHILD|WS_CLIPCHILDREN|WS_CLIPSIBLINGS));  // Set the window style
//		Throw(video_window->put_MessageDrain(OAHWND(hwnd)));                             // Set the destination for messages
//	}
//	else
//	{
//		Throw(video_window->put_Visible(FALSE));             // Make the window invisible
//		Throw(video_window->put_MessageDrain(OAHWND(hwnd))); // Set the destination for messages
//		Throw(video_window->put_Owner(0));                   // Unparent it
//	}
//}
		...
		//		if (pr::Succeeded(hr))
		//		{
		//			Throw(m_surfaces[0]->GetContainer(__uuidof(IDirect3DTexture9), (void**)&tex.m_ptr));
		//			info.Depth           = 1;
		...
		//				D3DDISPLAYMODE dm; Throw(m_d3d_device->GetDisplayMode(0, &dm));
		...
		//				// Copy the full surface onto the texture's surface
		//				D3DPtr<IDirect3DSurface9> surface = m_video.m_tex->surf(0);
		//				Throw(m_d3d_device->StretchRect(lpPresInfo->lpSurf, 0, surface.m_ptr, 0, D3DTEXF_NONE));
		//			}
		//			// We have got the textures allocated by VMR, all we need to do is to get them from the surface
		//			else if (!m_local_tex)
		//			{
		//				//todo: m_video.m_texture = 0;
		//				//Throw(lpPresInfo->lpSurf->GetContainer(__uuidof(IDirect3DTexture9), (void**)&m_video.m_texture.m_ptr));
		//			}
		...
		//		D3DDEVICE_CREATION_PARAMETERS params;
		//		Throw(m_d3d_device->GetCreationParameters(&params));
		//		Throw(m_d3d_device->GetDirect3D(&d3d.m_ptr));
