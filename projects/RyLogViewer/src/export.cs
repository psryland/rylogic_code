...
namespace RyLogViewer
{
	public partial class Main
	{
		...
		private bool DoExportWithProgress(string filepath, IEnumerable<Range> ranges, string row_delimiter, string col_delimiter, StreamWriter outp)
		{
			...
			var export = new ProgressForm("Exporting...", null, null, ProgressBarStyle.Continuous, (s,a,cb)=>
				{
					...
					// Do the export
					DoExport(filepath ,ranges ,row_delimiter ,col_delimiter ,outp ,report_progress);
		...
		private void DoExport(string filepath, IEnumerable<Range> ranges, string row_delimiter, string col_delimiter, StreamWriter outp, ProgressFunc report_progress)
		{
			using (var file = LoadFile(filepath))
			{
				...
				foreach (var rng in ranges)
				{
					...
					var r = new Range(Maths.Clamp(rng.Begin, 0, file.Length), Maths.Clamp(rng.End  , 0, file.Length));
