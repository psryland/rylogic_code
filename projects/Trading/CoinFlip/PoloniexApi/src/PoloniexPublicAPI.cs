...
namespace Poloniex.API
{
	public class PoloniexApiPublic :IDisposable
	{
		...
		public async Task<Dictionary<string, PriceData>> GetTradePairs()
		{
			...
			return await GetData<Dictionary<string, PriceData>>("returnTicker");
		}
		...
		public async Task<Dictionary<string, OrderBook>> GetOrderBook(int depth)
		{
			...
			return await GetData<Dictionary<string, OrderBook>>("returnOrderBook", "currencyPair=all", "depth="+depth);
		}
		...
		public async Task<OrderBook> GetOrderBook(CurrencyPair pair, int depth)
		{
			...
			return await GetData<OrderBook>("returnOrderBook", "currencyPair="+pair.Id, "depth="+depth);
		}

		/// <summary>Return the trade history for the given pair</summary>
		public async Task<List<Trade>> GetTradeHistory(CurrencyPair pair)
		{
			// https://poloniex.com/public?command=returnTradeHistory&currencyPair=BTC_NXT
			return await GetData<List<Trade>>("returnTradeHistory", "currencyPair="+pair.Id);
		}
		public async Task<List<Trade>> GetTradeHistory(CurrencyPair pair, DateTimeOffset start_time, DateTimeOffset end_time)
		{
			// https://poloniex.com/public?command=returnTradeHistory&currencyPair=BTC_NXT&start=1410158341&end=1410499372
			return await GetData<List<Trade>>("returnTradeHistory", "currencyPair="+pair.Id, "start="+Misc.ToUnixTime(start_time), "end="+Misc.ToUnixTime(end_time));
		}

		/// <summary></summary>
		public async Task<List<MarketChartData>> GetChartData(CurrencyPair pair, MarketPeriod period, DateTimeOffset start_time, DateTimeOffset end_time)
		{
			return await GetData<List<MarketChartData>>("returnChartData", "currencyPair="+pair.Id, "start="+Misc.ToUnixTime(start_time), "end="+Misc.ToUnixTime(end_time), "period="+(int)period);
		}
