	...
	public class ChartUI :BaseUI
	{
			...
			:base(model, "Chart")
		{
			try
			{
				...
				ChartCtrl = new ChartControl(string.Empty, new Settings.ChartSettings().Style);
		...
		protected override void SetModelCore(Model model)
		{
			if (Model != null)
			{
				Model.PairsUpdated -= HandlePairsUpdated;
			}
		...
		public Settings.ChartSettings ChartSettings
		{
			...
			private set
			{
				...
				if (m_chart_settings != null)
				{
					...
					m_tsc.LoadLocations(m_chart_settings.ToolStripLayout);
					m_chart_settings.PropertyChanged += HandleSettingChanged;
				}
				...
				{
					// Force a settings save when the chart settings change
					Model.Settings.Save();
				}
			}
		}
		...
		public Instrument Instrument
		{
			...
			private set
			{
				...
				if (m_instrument != null)
				{
					...
					m_instrument.DataSyncingChanged -= Invalidate;
		...
		/// <summary>Auto range this chart</summary>
		...
		private void SetupUI()
		{
			...
			// Horizontal line
			m_btn_vert_line.ToolTipText = "Add a vertical line";
			m_btn_vert_line.CheckOnClick = false;
			m_btn_vert_line.Click += (s,a) =>
			{
				m_btn_vert_line.Checked = true;
				ChartCtrl.MouseOperations.SetPending(MouseButtons.Left, new DropNewIndicatorOp(this, CreateIndy, () => m_btn_vert_line.Checked = false));
				IndicatorVertLine CreateIndy(ChartControl.HitTestResult hit)
				{
					return new IndicatorVertLine{ FIndex = hit.ChartPoint.X };
				}
			};
			...
			m_tsc.AutoPersistLocations(ChartSettings.ToolStripLayout, loc => ChartSettings.ToolStripLayout = loc);
		}
		...
		public void SetInstrument(TradePair pair, ETimeFrame time_frame)
		{
		...
		private string HandleChartXAxisLabels(double x, double step)
		{
			...
			var first_tick = curr == first || prev < first || x - step < ChartCtrl.XAxis.Min;
		...
		private void HandleAddOverlaysOnPaint(object sender, ChartControl.AddOverlaysOnPaintEventArgs e)
		{
			...
			var pair = Model.FindPairOnCurrentExchange(Instrument.Pair.Name) ?? Instrument.Pair;
			...
			{
				var price     = (float)(decimal)(spot_q2b ?? 0m);
			...
			{
				var price     = (float)(decimal)(spot_b2q ?? 0m);
				var price_str = e.Chart.YAxis.TickText(price, 0.0);
				var pt        = e.Chart.ChartToClient(new PointF((float)e.Chart.XAxis.Min, price));
				var sz        = e.Gfx.MeasureString(price_str, e.Chart.YAxis.Options.TickFont);
				var box       = sz.Scaled(1.05f);
			...
			if (e.Chart.XAxis.Range.Contains(Instrument.Count - 1))
			{
				...
				var pt = e.Chart.ChartToClient(new PointF(Instrument.Count - 1, (float)e.Chart.YAxis.Min));
				pt.X = (int)(pt.X - sz.Width / 2);
				pt.Y = (int)(pt.Y + 5.0);

				// Draw the string under the candle
				e.Gfx.FillRectangle(Brushes.DarkGray, new RectangleF(pt.X, pt.Y, sz.Width, sz.Height));
		...
		private void HandleAutoRanging(object sender, ChartControl.AutoRangeEventArgs e)
		{
			...
			var bb = BBox.Reset;
			var width = DisplayRectangle.Width / 6; // in candles
		...
		{
			args.Window.RemoveObjects(new[] { CtxId }, 1, 0);
			if (Instrument == null)
				return;
			...
			{
				// Updating text
				if (GfxUpdatingText != null && Instrument.DataSyncing)
					args.AddToScene(GfxUpdatingText);
			}
			#endregion
		...
		public event EventHandler<ChartControl.ChartRenderingEventArgs> Rendering;
		...
		private static readonly Guid CtxId = Guid.NewGuid();
		...
		private abstract class GfxObject :GfxBuffers
		{
			...
			public View3d.Object Gfx
			{
				get
				{
					...
					{
						m_update_in_progerss = m_update_issue;
						UpdateGfx();
					}
					return m_gfx;
				}
			...
			{
				get { return m_update_issue != m_update_in_progerss; }
			}
			protected int m_update_in_progerss;
			protected int m_update_issue;
		...
		private class GfxObjectCandles :GfxBuffers
		{
			...
			private CandleGfx At(int cache_idx)
			{
				...
				return m_cache.Get(cache_idx, i =>
				{
					...
					var colour_bullish = m_chart.ChartSettings.AskColour.ToArgbU();
					var colour_bearish = m_chart.ChartSettings.BidColour.ToArgbU();
		...
		private class GfxObjectMarketDepth :GfxObject
		{
			...
			{
				// Copy values in the main thread
				...
				ThreadPool.QueueUserWorkItem(_ =>
				{
					...
					m_chart.Model.RunOnGuiThread(() =>
					{
						...
						for (int i = 0; i != count_b2q; ++i)
						{
							...
							var hx = (float)(width_scale * volume / 2);
							var v = vert;
		...
		private class GfxObjectPositions :IDisposable
		{
			...
			public View3d.Object Get(Position pos)
			{
				...
				var gfx = m_cache.Get(pos.UniqueKey, g => new View3d.Object($"*Line Position_{pos.OrderId} FF8080FF {{ 0 0 0 1 0 0 }}", false, CtxId, null));
				var x = instrument.IndexAt(new TimeFrameTime(pos.Created.Value, instrument.TimeFrame));
				var y = (float)(decimal)pos.PriceQ2B;
		...
		private View3d.Object GfxUpdatingText
		{
		...
		private View3d.Object m_gfx_updating_text;
		...
		private void InitializeComponent()
		{
			...
			this.m_il_toolbar_btns.Images.SetKeyName(1, "market_depth.png");
			this.m_il_toolbar_btns.Images.SetKeyName(2, "clock_back.png");
