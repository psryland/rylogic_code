...
namespace CoinFlip
{
	...
	public class TradePair :IComparable<TradePair>, IComparable
	{
		...
		public RangeF<Unit<decimal>> VolumeRangeBase { get; private set; }

		/// <summary>The allowable range of volume for trading the quote currency</summary>
		public RangeF<Unit<decimal>> VolumeRangeQuote { get; private set; }
		...
		/// <summary>Return the default volume to use for a trade on this pair (in Base currency)</summary>
		...
		public RangeF<Unit<decimal>> VolumeRangeIn(ETradeType tt)
		{
			return
				tt == ETradeType.B2Q ? VolumeRangeBase :
				tt == ETradeType.Q2B ? VolumeRangeQuote :
				throw new Exception("Unknown trade type");
		}

		/// <summary>Return the volume range to validate against for the output currency</summary>
		public RangeF<Unit<decimal>> VolumeRangeOut(ETradeType tt)
		{
			return
				tt == ETradeType.B2Q ? VolumeRangeQuote :
				tt == ETradeType.Q2B ? VolumeRangeBase :
				throw new Exception("Unknown trade type");
		}
