...
#undef PR_PH_TEST_ANGULAR
	...
	// Update the broadphase now that the rigid body has moved
...
void EvolveAngularOrder1(Rigidbody& rb, float elapsed_seconds)
{
	...
	cast_m3x3(rb.m_object_to_world) += CrossProductMatrix3x3(rb.AngVelocity() * elapsed_seconds) * rb.Orientation();
}
...
void EvolveAngularOrder2(Rigidbody& rb, float elapsed_seconds)
{
	...
	m3x3 mid_orientation            = rb.Orientation() + CrossProductMatrix3x3(rb.AngVelocity() * half_dt) * rb.Orientation();
	m3x3 mid_ws_inv_inertia_tensor  = mid_orientation * rb.m_os_inv_inertia_tensor * GetTranspose(mid_orientation);
	...
	cast_m3x3(rb.m_object_to_world) += CrossProductMatrix3x3(mid_ang_velocity * elapsed_seconds) * rb.Orientation();
}
...
void EvolveAngularOrder5(Rigidbody& rb, float elapsed_seconds)
{
	...
	m3x3    step_orientation;
	m3x3    step_ws_inv_inertia_tensor;
	...
	m3x3    orientation_0  = rb.Orientation();
	m3x3    dorientation_0 = CrossProductMatrix3x3(ang_velocity_0) * orientation_0;
	...
	m3x3 dorientation_1         = CrossProductMatrix3x3(step_ang_velocity) * step_orientation;
	...
	step_ws_inv_inertia_tensor  = rb.Orientation() * rb.m_os_inv_inertia_tensor * GetTranspose(rb.Orientation());
	step_ang_velocity           = step_ws_inv_inertia_tensor * rb.m_inv_mass * step_ang_momentum;
	m3x3 dorientation_2         = CrossProductMatrix3x3(step_ang_velocity) * step_orientation;
	...
	step_ws_inv_inertia_tensor  = rb.Orientation() * rb.m_os_inv_inertia_tensor * GetTranspose(rb.Orientation());
	step_ang_velocity           = step_ws_inv_inertia_tensor * rb.m_inv_mass * step_ang_momentum;
	m3x3 dorientation_3         = CrossProductMatrix3x3(step_ang_velocity) * step_orientation;
	...
	step_ws_inv_inertia_tensor  = rb.Orientation() * rb.m_os_inv_inertia_tensor * GetTranspose(rb.Orientation());
	step_ang_velocity           = step_ws_inv_inertia_tensor * rb.m_inv_mass * step_ang_momentum;
	m3x3 dorientation_4         = CrossProductMatrix3x3(step_ang_velocity) * step_orientation;
	...
	step_ws_inv_inertia_tensor  = rb.Orientation() * rb.m_os_inv_inertia_tensor * GetTranspose(rb.Orientation());
	step_ang_velocity           = step_ws_inv_inertia_tensor * rb.m_inv_mass * step_ang_momentum;
	m3x3 dorientation_5         = CrossProductMatrix3x3(step_ang_velocity) * step_orientation;
	...
	cast_m3x3(rb.m_object_to_world) += dorientation_0*step0 + dorientation_2*step2 + dorientation_3*step3 + dorientation_5*step5;
}
