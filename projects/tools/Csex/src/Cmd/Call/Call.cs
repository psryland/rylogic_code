	...
	public class Call :Cmd
	{
		...
		public override bool CmdLineOption(string option, string[] args, ref int arg)
		{
			switch (option)
			{
				...
				default:
				{
					return base.CmdLineOption(option, args, ref arg);
				}
			}
		}
		...
		{
			using var ass_resolve = Scope.Create(
				() => AppDomain.CurrentDomain.AssemblyResolve += HandleAssemblyResolve,
				() => AppDomain.CurrentDomain.AssemblyResolve -= HandleAssemblyResolve);

			// Load the assembly
			var ass = Assembly.LoadFile(m_ass) ?? throw new Exception($"Failed to load assembly: {m_ass}");

			// Find the type
			var ty = ass.GetExportedTypes().FirstOrDefault(x => string.Compare(x.FullName, m_typename, m_ignore_case) == 0) ?? throw new Exception($"No type called {m_typename} found");

			// Find the static method
			var mi = ty.GetMethod(m_method, BindingFlags.Public | BindingFlags.Static) ?? throw new Exception($"No method called {m_method} on {m_typename} found");
