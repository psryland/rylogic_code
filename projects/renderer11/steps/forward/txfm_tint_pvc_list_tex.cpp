...
namespace pr
{
	namespace rdr
	{
		...
		#include "renderer11/shaders/hlsl/compiled/txfm_tint_pvc_lit_tex.vs.h"
		#include "renderer11/shaders/hlsl/compiled/txfm_tint_pvc_lit_tex.ps.h"
		...
		// Create this shader
		template <> void ShaderManager::CreateShader<TxTintPvcLitTex>()
		{
			// Create the shader
			VShaderDesc vsdesc(txfm_tint_pvc_lit_tex_vs, VertPCNT());
			PShaderDesc psdesc(txfm_tint_pvc_lit_tex_ps);
			CreateShader<TxTintPvcLitTex>(EStockShader::TxTintPvcLitTex, &vsdesc, &psdesc, "txfm_tint_pvc_lit_tex");
		}
	}
}
