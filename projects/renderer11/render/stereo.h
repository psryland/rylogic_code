...
namespace pr
{
	namespace rdr
	{
		...
		struct Stereo
		{
			...
			D3DPtr<ID3D11Texture2D> m_rt_tex;     // The offscreen rendertarget used to render the left and right views into
			D3DPtr<ID3D11RenderTargetView> m_rtv; // A render target view of 'm_rt_tex'
			D3DPtr<ID3D11Texture2D> m_ds_tex;     // The offscreen depth stencil buffer used to render the left and right views into
			...
			void BlitRTV(D3DPtr<ID3D11DeviceContext>& dc) const;

			// An RAII object for managing setup/teardown when rendering a stereoscopic scene
			struct RenderScope
			{
