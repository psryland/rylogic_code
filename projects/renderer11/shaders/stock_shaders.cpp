	...
	namespace rdr
	{
		...
		struct TxTint :BaseShader
		{
			static void Create(ShaderManager& sm, D3DPtr<ID3D11Device>& device)
			{
				...
				ShaderPtr shdr = sm.CreateShader<TxTint>(EShader::TxTint, TxTint::Setup, &vsdesc, &psdesc, "txfm_tint");
				CreateCBufModel(device, shdr->m_cbuf);
			}
			static void Setup(D3DPtr<ID3D11DeviceContext>& dc, Nugget const& nugget, BaseInstance const& inst, RenderStep const& rstep)
			{
				...
				CBufModel cb = {};
				Txfm(inst, rstep.m_scene->m_view, cb);
				Tint(inst, cb);
				{
		...
		struct TxTintPvc :BaseShader
		{
			static void Create(ShaderManager& sm, D3DPtr<ID3D11Device>& device)
			{
				...
				ShaderPtr shdr = sm.CreateShader<TxTintPvc>(EShader::TxTintPvc, TxTintPvc::Setup, &vsdesc, &psdesc, "txfm_tint_pvc");
				CreateCBufModel(device, shdr->m_cbuf);
			}
			static void Setup(D3DPtr<ID3D11DeviceContext>& dc, Nugget const& nugget, BaseInstance const& inst, RenderStep const& rstep)
			{
