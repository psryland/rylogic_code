	...
	namespace audio
	{
		...
		void WaveFindFormatAndData(uint8_t const* wav_data, size_t wav_data_size, WAVEFORMATEX const*& wfx, uint8_t const*& data, uint32_t& data_size, bool& dpds, bool& seek)
		{
			...
			if (wav_data_size < sizeof(RIFFChunk)*2 + sizeof(uint32_t) + sizeof(WAVEFORMAT))
				throw std::exception("WaveFindFormatAndData: Insufficient data.");

			// Locate RIFF 'WAVE'
			auto riff_chunk = FindChunk(wav_data, wav_data_size, FOURCC_RIFF_TAG);
			if (!riff_chunk || riff_chunk->size < 4)
				throw std::exception("RIFF chunk not found");
			...
			if (riff_header->riff != FOURCC_WAVE_FILE_TAG && riff_header->riff != FOURCC_XWMA_FILE_TAG)
				throw std::exception("RIFF chunk header not valid");
			...
			if (ptr + sizeof(RIFFChunk) > wav_data_end)
				throw std::exception("RIFF chunk header incomplete");
			...
			if (!fmt_chunk || fmt_chunk->size < sizeof(PCMWAVEFORMAT))
				throw std::exception("Format chunk not found");
			...
			if (ptr + fmt_chunk->size > wav_data_end)
				throw std::exception("Format chunk header incomplete");
			...
			default:
				{
					if (fmt_chunk->size < sizeof(WAVEFORMATEX) ||
						...
						throw std::exception("WAVEFORMATEX chunk is invalid");

					switch ((EWaveFormat)wf->wFormatTag)
					{
					default:
						{
							throw std::exception(pr::FmtS("'wav_data' is not a supported type: %X", wf->wFormatTag));
						}
					...
					case  EWaveFormat::XMA2: // XMA2 is supported by XBox One
						{
							...
							if (fmt_chunk->size < sizeof_XMA2WAVEFORMATEX || wf->cbSize < sizeof_XMA2WAVEFORMATEX - sizeof_WAVEFORMATEX)
								throw std::exception("'wav_data' of format 'EWaveFormat::XMA2' is incomplete");
					...
					case EWaveFormat::ADPCM:
						{
							if (fmt_chunk->size < sizeof(WAVEFORMATEX) + MSADPCM_FORMAT_EXTRA_BYTES || wf->cbSize < MSADPCM_FORMAT_EXTRA_BYTES)
								throw std::exception("'wav_data' of format 'EWaveFormat::ADPCM' is incomplete");
					...
					case EWaveFormat::EXTENSIBLE:
						{
							if (fmt_chunk->size < sizeof(WAVEFORMATEXTENSIBLE) || wf->cbSize < sizeof(WAVEFORMATEXTENSIBLE) - sizeof(WAVEFORMATEX))
								throw std::exception("'wav_data' of format 'EWaveFormat::EXTENSIBLE' is incomplete");
							...
							if (memcmp(sub_format, wfxe_base, sizeof(GUID) - sizeof(DWORD)) != 0)
								throw std::exception("'wav_data' of format 'EWaveFormat::EXTENSIBLE' has an unsupported sub format");

							// Parse the sub format
							switch ((EWaveFormat)wfex->SubFormat.Data1)
							{
							default:
								throw std::exception(pr::FmtS("'wav_data' of format 'EWaveFormat::EXTENSIBLE' has an unsupported sub format: %X", wfex->SubFormat.Data1));
							case EWaveFormat::PCM:
							case EWaveFormat::IEEE_FLOAT:
								break;
			...
			if (ptr + data_chunk->size > wav_data_end)
				throw std::exception("Wave data chunk is incomplete");
		...
		void WaveFindLoopInfo(uint8_t const* wav_data, size_t wav_data_size, uint32_t& loop_start, uint32_t& loop_length)
		{
			...
			if (riff_header->riff != FOURCC_WAVE_FILE_TAG)
				throw std::exception("RIFF chunk does not have the wave file tag");
			...
			if (dls_chunk)
			{
				...
				if (ptr + dls_chunk->size > wav_data_end)
					throw std::exception("DLS chunk is incomplete");
			...
			if (midi_chunk)
			{
				...
				if (ptr + midi_chunk->size > wav_data_end)
					throw std::exception("MIDI chunk is incomplete");
		...
		void WaveFindTable(uint8_t const* wav_data, size_t wav_data_size, uint32_t tag, uint32_t const*& data, uint32_t& data_count)
		{
			...
			if (wav_data_size < sizeof(RIFFChunk) + sizeof(uint32_t))
				throw std::exception("WaveFindTable: Insufficient data.");
			...
			if (table_chunk)
			{
				...
				if (ptr + table_chunk->size > wav_data_end)
					throw std::exception("Table chunk is incomplete");

				if ((table_chunk->size % sizeof(uint32_t)) != 0)
					throw std::exception("Table chunk is invalid (not a multiple of 4 bytes)");
		...
		void LoadAudioFromFile(wchar_t const* file_path, std::unique_ptr<uint8_t[]>& wav_data, DWORD& bytes_read)
		{
			...
			auto safe_handle = [](HANDLE h) -> HANDLE { return (h != INVALID_HANDLE_VALUE) ? h : nullptr; };
			using ScopedHandle = std::unique_ptr<void, handle_closer>;

			#if (_WIN32_WINNT >= _WIN32_WINNT_WIN8)
			ScopedHandle hFile(safe_handle(CreateFile2(file_path, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, nullptr)));
			#else
			ScopedHandle hFile(safe_handle(CreateFileW(file_path, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr)));
			#endif
			if (hFile == nullptr)
			...
			#if (_WIN32_WINNT >= _WIN32_WINNT_VISTA)
			{
				...
				if (!GetFileInformationByHandleEx(hFile.get(), FileStandardInfo, &fileInfo, sizeof(fileInfo)))
					pr::Throw(HRESULT_FROM_WIN32(GetLastError()));
				file_size = fileInfo.EndOfFile;
			}
			#else
			GetFileSizeEx(hFile.get(), &file_size);
			#endif

			// File is too big for 32-bit allocation, so reject read
			if (file_size.HighPart > 0)
				throw std::exception("Audio file is too big for 32-bit allocation");

			// Need at least enough data to have a valid minimal WAV file
			if (file_size.LowPart < sizeof(RIFFChunk) * 2 + sizeof(DWORD) + sizeof(WAVEFORMAT))
				throw std::exception("LoadAudioFromFile: Insufficient data.");

			// Create enough space for the file data
			wav_data.reset(new (std::nothrow) uint8_t[file_size.LowPart]);
			if (!wav_data)
				throw std::exception(pr::FmtS("LoadAudioFromFile: Allocation of %u bytes failed", file_size.LowPart));

			// Read the data into the buffer
			if (!ReadFile(hFile.get(), wav_data.get(), file_size.LowPart, &bytes_read, nullptr))
				pr::Throw(HRESULT_FROM_WIN32(GetLastError()));
			if (bytes_read < file_size.LowPart)
				throw std::exception(pr::FmtS("LoadAudioFromFile: Partial read of wave file. Expected %u bytes, read %u bytes", file_size.LowPart, bytes_read));
		}
		...
		{
			if (wav_data == nullptr)
				throw std::exception("'wav_data' cannot be null");
			...
			if (wav_data_size < sizeof(RIFFChunk) * 2 + sizeof(DWORD) + sizeof(WAVEFORMAT))
				throw std::exception("LoadWAVAudioInMemory: Insufficient data.");
		...
		{
			if (filepath == nullptr)
				throw std::exception("'filepath' cannot be null");
