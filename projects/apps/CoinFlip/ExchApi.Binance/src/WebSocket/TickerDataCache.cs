			...
			get
			{
				...
				try
				{
					...
					lock (stream.TickerData)
					{
						...
						return idx >= 0 ? new Ticker(stream.TickerData[idx]) : throw new Exception($"No ticker found for pair {pair}");
					}
				}
				catch (Exception ex)
				{
			...
			public WebSocket Socket
			{
				...
				private set
				{
					...
					void HandleMessage(object? sender, WebSocket.MessageEventArgs e)
					{
						try
						{
							var update = JToken.Parse(e.Text).ToObject<List<TickerUpdate>>() ?? throw new Exception("Expected ticker update");
							ApplyUpdate(update);
						}
