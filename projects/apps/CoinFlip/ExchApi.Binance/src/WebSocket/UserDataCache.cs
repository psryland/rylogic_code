...
namespace Binance.API
{
	public sealed class UserDataCache :IDisposable
	{
		...
		private class UserDataStream :IDisposable
		{
				...
				private set
				{
					...
					void HandleMessage(object? sender, WebSocket.MessageEventArgs e)
					{
						try
						{
							...
							var event_type = jtok["e"]?.Value<string>() ?? String.Empty;
							switch (event_type)
							{
								case WebSocketEvent.EventName.AccountInfo:
								{
									ApplyUpdate(jtok.ToObject<AccountUpdate>() ?? throw new Exception($"Failed to parse object to '{nameof(AccountUpdate)}'"));
									break;
								}
								case WebSocketEvent.EventName.AccountPosition:
								{
									ApplyUpdate(jtok.ToObject<AccountBalanceUpdate>() ?? throw new Exception($"Failed to parse object to '{nameof(AccountBalanceUpdate)}'"));
									break;
								}
								case WebSocketEvent.EventName.OrderTrade:
								{
									ApplyUpdate(jtok.ToObject<TradeOrderUpdate>() ?? throw new Exception($"Failed to parse object to '{nameof(TradeOrderUpdate)}'"));
									break;
								}
								default:
								{
			...
			{
				if (update.Balances is not IList<AccountUpdate.BalanceData> bals) return;
				var map = bals.Where(x => x.Asset != null).ToDictionary(x => x.Asset!, x => x);

				lock (m_balance_data)
				{
					foreach (var bal in m_balance_data.Balances)
					{
						if (bal.Asset is string asset && map.TryGetValue(asset, out var upd))
						{
							bal.Free = upd.Free;
