...
namespace CoinFlip.UI.Dialogs
{
	public partial class FundAllocationsUI : Window, IDisposable, INotifyPropertyChanged
	{
		...
		public FundAllocationsUI(Window owner, Model model)
		{
			...
			Funds = new ListCollectionView(model.Funds) { Filter = x => ((Fund)x).Id != Fund.Main.Id };
		...
		private class CoinDataAdapter :INotifyPropertyChanged, IValueTotalAvail
		{
			...
			public decimal Total => Coin?.Balances.Sum(x => x.Total) ?? 0m;

			/// <summary>The available amount of the coin (in coin currency)</summary>
			public decimal Available => Coin?.Balances.Sum(x => x.Available) ?? 0m;
			...
			public decimal Allocated
			{
				get
				{
					...
					return Coin?.Balances[fund].Total ?? 0m;
				}
				set
				{
					var fund = SelectedFund;
					...
					NotifyPropertyChanged(nameof(MaxAvailable));
					m_owner.NotifyPropertyChanged(nameof(Validate));
				}
			}
