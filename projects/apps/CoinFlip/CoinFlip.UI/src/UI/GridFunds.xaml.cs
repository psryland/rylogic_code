	...
	public partial class GridFunds : Grid, IDockable, IDisposable
	{
		...
		public Model Model
		{
			...
			set
			{
				...
				{
					if (e.Before) return;
					HandleFundsChanged(null, new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Reset));
				}
		...
		public bool HasUserFunds => Funds.Cast<Fund>().Any(x => x != Fund.Main);
		...
		private void RemoveFundInternal()
		{
			...
			dlg.Items.AddRange(Model.Funds.Where(x => x != Fund.Main));
			if (dlg.ShowDialog() == true)
			{
