...
namespace CoinFlip.UI
{
	public partial class GridCoins :Grid, IDockable, IDisposable, INotifyPropertyChanged
	{
		...
		{
			get => CoinsView.CurrentAs<CoinDataAdapter>();
		...
		public CoinFilter? Filter
		{
			...
			set
			{
				...
				{
					if (sender is not CoinFilter filter) throw new ArgumentNullException(nameof(sender));
		...
		{
			if (Model.Simulation is not Simulation sim)
				return;
			...
			private IEnumerable<Exchange> SourceExchanges
			{
				get
				{
					...
					else if (src == SpecialExchangeSources.Current)
					{
						...
						if (exch != null && exch is not CrossExchange)
							return new[] { exch };
					}
					else
					{
