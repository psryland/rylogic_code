...
namespace CoinFlip
{
	...
	public sealed class PriceData :IDisposable
	{
		...
		/// <summary>Get the current that would be 'Current' at the given time (or null if no data)</summary>
		...
		private void NotifyDataChanged(DataEventArgs args) => DataChanged?.Invoke(this, args);
		...
		private CancellationTokenSource m_update_thread_cancel = null!;
		private ManualResetEvent m_update_thread_exit = null!;
		...
		private void Add(ETimeFrame tf, Candle candle)
		{
			...
			switch (update_type)
			{
				...
				{
					if (m_newest == null) throw new NullReferenceException("No Current candle to update");
			...
			NotifyDataChanged(new DataEventArgs(update_type, this, new RangeI(Count-1, Count), candle));
		}
		...
		private void Add(ETimeFrame tf, IEnumerable<Candle> candles)
		{
			...
			// Notify data added/changed
			NotifyDataChanged(new DataEventArgs(this));
		}
		...
		public void SimulationUpdate(bool force_invalidate)
		{
			...
			switch (update_type)
			{
				case DataEventArgs.EUpdateType.New:
				{
					...
					NotifyDataChanged(new DataEventArgs(update_type, this, new RangeI(count - 1, count), Current));
					break;
				}
				case DataEventArgs.EUpdateType.Current:
				{
	...
	public class DataEventArgs :EventArgs
	{
		...
		public EUpdateType UpdateType { get; }
