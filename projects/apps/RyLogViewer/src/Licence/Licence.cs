...
namespace RyLogViewer
{
	...
	public class Licence
	{
			...
			: this()
		{
			...
			LicenceHolder  = root.Element(nameof(LicenceHolder )).As<string>(LicenceHolder);
			EmailAddress   = root.Element(nameof(EmailAddress  )).As<string>(EmailAddress);
			Company        = root.Element(nameof(Company       )).As<string>(Company);
			VersionMask    = root.Element(nameof(VersionMask   )).As<string>(VersionMask);
			ActivationCode = root.Element(nameof(ActivationCode)).As<string>(ActivationCode);
			Changed = false;
		}
