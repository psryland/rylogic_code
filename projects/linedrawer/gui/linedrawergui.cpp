...
ldr::MainGUI::MainGUI(LPTSTR cmdline)
	:base()
	,m_menu()
	,m_status()
	,m_recent_files()
	,m_saved_views()
	,m_store_ui(m_hWnd)
	,m_measure_tool_ui(ldr::ReadPoint, &m_main->m_cam, m_main->m_rdr, m_hWnd)
	,m_angle_tool_ui(ldr::ReadPoint, &m_main->m_cam, m_main->m_rdr, m_hWnd)
...
// Init Window
LRESULT ldr::MainGUI::OnCreate(LPCREATESTRUCT create)
{
	pr::Throw(base::OnCreate(create));

	// Create a step context for rendering
	m_msg_loop.AddStepContext("rdr main loop", [this](double){ m_main->DoRender(false); }, 60.0f, false);

	// Add a step context for stepping plugins
	m_msg_loop.AddStepContext("plugin step", [this](double){ m_main->m_plugin_mgr.Poll(); }, 30.0f, true);
...
void ldr::MainGUI::OnTimer(UINT_PTR)
{
...
// Paint the window
void ldr::MainGUI::OnPaint(HDC hDC)
{
	SetMsgHandled(FALSE);
...
void ldr::MainGUI::OnGetMinMaxInfo(LPMINMAXINFO lpMMI)
{
...
void ldr::MainGUI::OnDropFiles(HDROP hDropInfo)
{
...
void ldr::MainGUI::OnSysKeyDown(UINT nChar, UINT, UINT)
{
...
// Handle key presses
void ldr::MainGUI::OnKeyDown(UINT nChar, UINT, UINT)
{
	switch (nChar)
	{
	default:
...
void ldr::MainGUI::OnMouseDown(UINT flags, CPoint point)
{
...
void ldr::MainGUI::OnMouseUp(UINT flags, CPoint point)
{
	pr::v2 mouse_loc = pr::To<pr::v2>(point);
	int    btn_state = ButtonState(flags);
...
void ldr::MainGUI::OnMouseMove(UINT flags, CPoint point)
{
	pr::v2 mouse_loc = pr::To<pr::v2>(point);
	int    btn_state = ButtonState(flags);
...
BOOL ldr::MainGUI::OnMouseWheel(UINT, short delta, CPoint point)
{
	pr::v2 mouse_loc = pr::To<pr::v2>(point);
...
void ldr::MainGUI::OnMouseDblClk(UINT nFlags, CPoint point)
{
	pr::v2 mouse_loc = pr::To<pr::v2>(point);
...
LRESULT ldr::MainGUI::OnFileNew(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnFileNewScript(WORD, WORD, HWND, BOOL&)
{
	WTL::CFileDialog fd(FALSE,0,0,0,ldr::FileOpenFilter,m_hWnd);
...
LRESULT ldr::MainGUI::OnFileOpen(WORD, WORD, HWND, BOOL&)
{
	WTL::CFileDialog fd(TRUE,0,0,0,ldr::FileOpenFilter,m_hWnd);
	if (fd.DoModal() != IDOK) return S_OK;
...
LRESULT ldr::MainGUI::OnFileOpenAdditive(WORD, WORD, HWND, BOOL&)
{
	CFileDialog fd(TRUE,0,0,0,ldr::FileOpenFilter,m_hWnd);
	if (fd.DoModal() != IDOK) return S_OK;
...
LRESULT ldr::MainGUI::OnFileShowOptions(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnFilePluginMgr(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnAppClose(WORD, WORD wID, HWND, BOOL&)
{
	CloseApp(wID);
	return S_OK;
}
...
LRESULT ldr::MainGUI::OnResetView(WORD, WORD wID, HWND, BOOL&)
{
	switch (wID)
	{
	default:
...
LRESULT ldr::MainGUI::OnViewAxis(WORD, WORD wID, HWND, BOOL&)
{
	pr::v4 axis;
	switch (wID)
	{
...
LRESULT ldr::MainGUI::OnSetFocusPosition(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnNavAlign(WORD, WORD wID, HWND, BOOL&)
{
	switch (wID)
	{
	default:
...
LRESULT ldr::MainGUI::OnSaveView(WORD, WORD wID, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnOrbit(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnShowObjectManagerUI(WORD, WORD, HWND, BOOL&)
{
	m_store_ui.Show(true);
	return S_OK;
}
...
LRESULT ldr::MainGUI::OnEditSourceFiles(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnDataClearScene(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnDataAutoRefresh(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnCreateDemoScene(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnShowFocus(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnShowOrigin(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnShowSelection(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnShowObjBBoxes(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnToggleFillMode(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnRender2D(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnShowLightingDlg(WORD, WORD, HWND, BOOL&)
{
	struct PreviewLighting
	{
		ldr::Main* m_main;
		PreviewLighting(ldr::Main* main) :m_main(main) {}
...
LRESULT ldr::MainGUI::OnShowToolDlg(WORD, WORD wID, HWND, BOOL&)
{
	switch (wID)
	{
...
LRESULT ldr::MainGUI::OnWindowAlwaysOnTop(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnWindowBackgroundColour(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnWindowExampleScript(WORD, WORD, HWND, BOOL&)
{
...
LRESULT ldr::MainGUI::OnCheckForUpdates(WORD, WORD, HWND, BOOL&)
{
	std::string version;
...
// Show the about box
LRESULT ldr::MainGUI::OnWindowShowAboutBox(WORD, WORD, HWND, BOOL&)
{
	ShowAbout();
	return S_OK;
}
...
// Shut the app down
void ldr::MainGUI::CloseApp(int exit_code)
{
	DestroyWindow();
...
// Create a new file
void ldr::MainGUI::FileNew(char const* filepath)
{
...
void ldr::MainGUI::FileOpen(char const* filepath, bool additive)
{
...
void ldr::MainGUI::OpenTextEditor(StrList const& files)
{
...
void ldr::MainGUI::UpdateUI()
{
	// Camera orbit
	CheckMenuItem(GetMenu(), ID_NAVIGATION_ORBIT ,m_main->m_settings.m_CameraOrbit ? MF_CHECKED : MF_UNCHECKED);
...
void ldr::MainGUI::MouseStatusUpdate(pr::v2 const& mouse_location)
{
...
void ldr::MainGUI::ShowAbout() const
{
...
void ldr::MainGUI::MenuList_OnClick(pr::gui::MenuList* sender, pr::gui::MenuList::Item const& item)
{
...
void ldr::MainGUI::MenuList_ListChanged(pr::gui::MenuList* sender)
{
	if (static_cast<pr::gui::RecentFiles*>(sender) == &m_recent_files)
	{
...
// Handle info events
void ldr::MainGUI::OnEvent(ldr::Event_Info const& e)
{
	(void)e;
...
void ldr::MainGUI::OnEvent(ldr::Event_Warn const& e)
{
	(void)e;
	PR_INFO(PR_DBG_LDR, e.m_msg.c_str());
}
...
// Handle error events
void ldr::MainGUI::OnEvent(ldr::Event_Error const& e)
{
	if (!m_main || m_main->m_settings.m_ErrorOutputMsgBox)
		::MessageBoxA(m_hWnd, e.m_msg.c_str(), pr::FmtS("%s Error", ldr::AppTitleA()), MB_OK|MB_ICONERROR);
	else
	{} // error msg on status line
}

// Status text update
void ldr::MainGUI::OnEvent(ldr::Event_Status const& e)
{
...
void ldr::MainGUI::OnEvent(ldr::Event_Refresh const&)
{
	m_main->RenderNeeded();
}
void ldr::MainGUI::OnEvent(pr::ldr::Evt_Refresh const&)
{
	m_main->RenderNeeded();
}

// The measure tool window was closed
void ldr::MainGUI::OnEvent(pr::ldr::Evt_LdrMeasureCloseWindow const&)
{
	UpdateUI();
	m_main->RenderNeeded();
}
...
void ldr::MainGUI::OnEvent(pr::ldr::Evt_LdrMeasureUpdate const&)
{
	m_main->RenderNeeded();
}

// The angle tool window was closed
void ldr::MainGUI::OnEvent(pr::ldr::Evt_LdrAngleDlgCloseWindow const&)
{
	UpdateUI();
	m_main->RenderNeeded();
}
...
void ldr::MainGUI::OnEvent(pr::ldr::Evt_LdrAngleDlgUpdate const&)
{
	m_main->RenderNeeded();
}

// A number of objects are about to be added
void ldr::MainGUI::OnEvent(pr::ldr::Evt_AddBegin const&)
{
	m_suspend_render = true;
}

// The last object in a group has been added
void ldr::MainGUI::OnEvent(pr::ldr::Evt_AddEnd const&)
{
	m_suspend_render = false;
	m_main->RenderNeeded();
}
...
void ldr::MainGUI::OnEvent(pr::settings::Evt<UserSettings> const& e)
{
	MessageBox(e.m_msg.c_str(), "Settings Error", MB_OK);
}

// Parse command line options
bool ldr::MainGUI::CmdLineOption(std::string const& option, pr::cmdline::TArgIter& arg, pr::cmdline::TArgIter arg_end)
{
