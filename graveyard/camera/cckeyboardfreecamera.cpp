...
DeviceSettings MakeDeviceSettings(dinput::Context context, HWND window_handle)
{
...
// Constructor
KeyboardFreeCamera::KeyboardFreeCamera(const CameraControllerSettings& settings)
:m_settings(settings)
,m_context(m_settings.m_app_instance)
,m_keyboard(m_context, MakeDeviceSettings(m_context, m_settings.m_window_handle))
{
	PR_ASSERT_STR(PR_DBG_CAMERA, m_settings.m_camera != 0, "You must provide a camera to control");
}

// Camera Update
void KeyboardFreeCamera::Step(float elapsed_seconds)
{
