...
class FragmentMain : Fragment(), PropertyChangeListener
{
	...
	override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View
	{
		...
		mBeats = listOf(mBinding.beat1, mBinding.beat2, mBinding.beat3, mBinding.beat4, mBinding.beat5, mBinding.beat6, mBinding.beat7)
	...
	override fun onViewCreated(view: View, savedInstanceState: Bundle?)
	{
		...
		mBinding.labelTempo.text = getString(R.string.tempo_bpm, mSettings.tempo)
		mBinding.sliderSpeed.progress = mSettings.tempo
		mBinding.sliderSpeed.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener
		{
	...
	override fun propertyChange(prop: PropertyChangeEvent?)
	{
		...
		if (prop?.propertyName == Updater.RUNNING)
		{
			...
			if (mUpdater.running)
			{
				mBinding.buttonStart.setText(R.string.stop)
			}
			else
			{
				mBinding.buttonStart.setText(R.string.start)
		...
		if (prop?.propertyName == Updater.NOTIFY_CHORD)
		{
			...
			mBinding.textChord.text = getString(R.string.key_and_chord, mUpdater.key, mUpdater.chord)
			mBinding.textChordNext.text = getString(R.string.key_and_chord, mUpdater.keyNext, mUpdater.chordNext)
